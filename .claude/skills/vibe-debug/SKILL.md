...
"$V" --debug-cmd dump_last_playlist       # {exists, rows, currentPath} — the container mirror (Application Support/<bundle id>/LastPlaylist.m3u) as it stands on disk, read back through the app so no host read of the container is needed. Written only at quit: a fresh session reports the LAST quit's until it quits. A relaunch with the setting on restores it parked: dump_state shows player.state "paused", ui.currentTime "0:00" and ui.nowPlayingWithheldForRestore true until the first real play
