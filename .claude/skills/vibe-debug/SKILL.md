...
**Time to first audio is recoverable exactly, without instrumentation.** `position` is the audio actually rendered, so at poll time `T` with position `P`, playback began at `T - P` — the answer does not depend on how coarsely you polled. Measured that way on a real device, a single open settles in ~25ms and audio flows ~40ms after the command, independent of file size (2.4MB to 202MB), container, and entry point (`open`, `play_index`, `file_drag_drop`). Note `dump_state`'s `player.state` reports the pending *intent*, so it reads `playing` during a cloud open that has not landed; use position movement, not the state string, to time a cloud open.
...
**Verb naming rule.** Families take a verb-kind prefix (`dump_*` reads state, `set_*` writes a knob, `clear_*`, `settings_*`, `scan_*`); everything else is named directly for its feature (`seek`, `open`, `convert_to_flac`, `reorder_begin`). When two verbs could share a word, the object goes in the name — `file_drag_*` simulates an external file drop, `reorder_*` the internal row drag, bare `drag` posts pointer events. Never a subsystem prefix (`playlist_*`, `transport_*`): the feature name is the namespace. Check a new verb against this before adding it.
...
"$V" --debug-cmd settings_open appearance     # {ok, pane, paneTitle, panes, frame, paneFrame, paneFillsTabView, key} — opens the Settings window, creating it, and selects a pane by identifier (general|playback|appearance|files|convert|advanced|about), index or displayed title; bare `settings_open` just opens it. See The settings window below
...
"$V" --debug-cmd play_index 3          # compact action summary — plays playlist row n, synchronously: the verb block_main chains, where `open` would land its play a turn later
"$V" --debug-cmd burst 200 7           # {ok, jumps, playlist} — <jumps> [<seed>]: seeded random play_index jumps, one per main-queue turn, capped at 5000. Replies at once and keeps firing, so the NEXT command lands mid-burst — the in-process race hunt the channel's ~80ms-per-command cadence (2.4s under TSan) cannot stage
"$V" --debug-cmd dump_metadata_progress # {total, parsed, attempted} — how far the metadata sweep has got: attempted counts rows a parse has landed on, parsed those with real metadata, so a file that failed to parse is not mistaken for one still waiting
"$V" --debug-cmd hang_open track.wav   # {ok, hangingBasename, hungOpens} — <basename>|release: opens of that basename block inside the uncancellable AVAudioFile call (stage 2 of a cloud open, the one provider failure set_fake_cloud cannot stage); `release` lets every held open through and stops matching new ones
...
"$V" --debug-cmd set_window_width 900  # {ok, frame, bodyWidth} — body width in points (pitch panel excluded); optional height-points and seconds (0..10), e.g. `set_window_width 1400 650 2`. Timed resizing uses public AppKit frame changes at 60 Hz, replies on completion, and needs no CGEvents. Exercises window layout/rendering, not AppKit's mouse-tracking/live-resize event lifecycle
"$V" --debug-cmd work_tally begin    # both platforms: starts a fresh counting window over the existing debug signposts
"$V" --debug-cmd work_tally end      # {active, label, elapsedMs, work:{name:{count,totalMs,maxMs}}}; closes it. Nested intervals overlap: do not add waveform_update to its waveform_target/path children. Times cover app-side work, not render-server/GPU time; use Instruments for presentation hitches
"$V" --debug-cmd measure_resize 600 1080 240 # one out-and-back width sweep at 60 Hz, 30–600 frames; restores the frame and returns process CPU seconds, synchronous layout mean/p95/max milliseconds, and signpost counts/times, including a 0.6s settle tail. Pause playback and keep style, track and visibility fixed for before/after comparisons; this is programmatic resizing, not an OS border drag.
...
"$V" --debug-cmd file_drag_hover 520 275  # {ok, well} — synthetic external-file drag-over at a window point (top-left origin, like click): drives the playlist drop zone's wells through the real FileDropDelegate path. NOT an event: a genuine NSDraggingSession can't be synthesized, so these call the delegate directly. well = replace|add|none = what a drop there would hit
"$V" --debug-cmd file_drag_drop 520 275 ~/Music/track.wav  # {ok, dropping, well} — completes the synthetic drag: expands + delivers the drop at that point (well routing: replace|add|none→replace), then tears the drag-over UI down. ABSOLUTE path; same sandbox caveat as open; poll dump_state
"$V" --debug-cmd file_drag_end            # {ok} — the drag left the window without a drop: back to the rest presentation
"$V" --debug-cmd reorder_begin 1 3   # {ok, rows} — starts a synthetic playlist ROW-REORDER drag on the given rows: the real NSTableViewDataSource choreography (writer + token per row, willBegin) with a stand-in NSDraggingInfo, since a genuine NSDraggingSession cannot be synthesized. The session SURVIVES across channel commands on purpose — begin, then mutate the playlist with any other verb, then update/drop: the mid-drag races (replace-all rejection, a converted-away dragged row dropping out) that no pointer can stage deterministically. AppKit's half — the drag threshold, which rows a gesture picks up, the insertion line, autoscroll — is NOT exercised; that stays a manual pointer check
"$V" --debug-cmd reorder_update 2    # {ok, slot, operation} — validateDrop at an insertion slot (0..count): "move" or "none". Sweep it to assert the insertion-line decision at every slot, which even a real pointer cannot pin
"$V" --debug-cmd reorder_drop 0      # {ok, slot, dropped} — validate then accept at the slot, then end the session (AppKit's ordering: a refused slot slides back rather than staying live). dropped:false = the drop was refused; poll dump_state for the resulting order
"$V" --debug-cmd reorder_cancel      # {ok} — end the live session without a drop, clearing the controller's token and retained tracks through the same ended call a real cancel takes
...
"$V" --debug-cmd save_playlist ~/Library/Containers/com.commonwealthrecordings.Vibe/Data/tmp/set.m3u  # {ok, path, tracks} — File > Save Playlist… without its panel: writes the whole playlist as extended M3U through the same write path, entries relative to the file's folder, and notes it in Open Recent. {"error": "playlist is empty"} on an empty list. The path must be writable by the sandboxed app (the container's tmp is, and the host can read it back); open the file with `open` to round-trip it
"$V" --debug-cmd dump_last_playlist       # {exists, rows, currentIndex} — the container mirror (Application Support/<bundle id>/LastPlaylist.m3u) as it stands on disk, read back through the app so no host read of the container is needed. Written only at quit: a fresh session reports the LAST quit's until it quits. A relaunch with the setting on restores it parked (player.state "paused", ui.currentTime "0:00") and publishes no Now Playing until the first real play
...
"$V" --debug-cmd set_reopen_playlist on   # {ok, reopenLastPlaylist} — <on|off>, Settings > General > Startup > Load last playlist on launch: writes it and requests the ReopenLastPlaylist live effect. Off deletes the container mirror at once; the mirror is written only when the app QUITS (the `quit` verb, and so `launch.sh`, run applicationWillTerminate:)
...
**TRAP: items gated on the player window being key — Save Playlist…, Play Selected Track, Remove from Playlist, and the chain-resolved Close — read `enabled: false` and `click_menu` refuses them whenever it is not**, which is the normal state once the terminal has taken focus back after `launch.sh`. Re-activate first with `open -a "$PWD/$APP"` (an absolute path; a relative one fails with "Unable to find application") and `dump_state`'s `window.keyWindow` flips to true. `click_menu menu_save_playlist` then raises the real powerbox save sheet, which no verb can dismiss — a posted `key esc` never reaches the remote view — so drive saves with `save_playlist` and leave the sheet to a human.
