...
All debug tooling is compiled in **debug builds only** (`Debug/DebugUtil.m`), and `Vibe.xcodeproj` is generated by XcodeGen (not checked in), so generate + build first. `make build` / `scripts/build.sh` write to `build/DerivedData` — exactly where `launch.sh` looks:
