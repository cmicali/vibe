...
"$V" --debug-cmd set_appearance dark # {ok, appearance} — light|dark|system, edits the working theme live (a fixed theme pins the window; system follows the OS)
...
"$V" --debug-cmd import_theme '"'"'{"name":"T","windowCornerRadius":0}'"'"' # {ok, imported, name, themeCount} — inline JSON (starts with "{") or a path the APP can read; adds a user theme through the same sanitize-and-store path the Settings importer uses, bypassing its powerbox sheet. The only scripted route to a font change
