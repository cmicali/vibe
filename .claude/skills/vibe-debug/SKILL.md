...
"$V" --debug-cmd dump_cloud_health     # {cloudParsesPending, cloudLaneHeld, priorityLane:{pending,yieldedUnderHold,inFlight,liveTokens,held}, scanLane:{pending,delayed,inFlight,liveTokens,stageOneFinished}, materialization:{claims,waiters,interactiveRunning,backgroundRunning,interactivePending,backgroundPending,metadataHolds,handleRuns,handleOpensInFlight,…}} — loader and path-wide lanes on BOTH platforms. Every live count, pending list and hold belongs empty/zero once a sweep has settled; stageOneFinished is historical state, not residue
