...
"$V" --debug-cmd import_theme '"'"'{"name":"T","windowCornerRadius":0}'"'"' # {ok, imported, name, themeCount} — inline JSON (starts with "{"), or a path the APP can read (container tmp works) to a .json or a theme ZIP (theme.json + a custom album-art image); the same sanitize-and-store path as the Settings importer, powerbox-free. The only scripted route to a font change
