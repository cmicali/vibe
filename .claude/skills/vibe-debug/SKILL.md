...
**The mac desktop widget has the same trap in a stricter form, and `VIBE_SIGN_MAC=1` is its flag.** The mac group is team-prefixed (`VibeWidgetState.m`'s TRAP) and the default `make build` is ad-hoc, which carries no team, so the extension is denied the container and draws its placeholder. `make build CONFIG=Debug VIBE_SIGN_MAC=1` signs with the Apple Development certificate; confirm with `codesign -dv <app> | grep TeamIdentifier` (mac signatures, unlike the simulator's, do carry the entitlements `codesign -d --entitlements -` prints). The container is `~/Library/Group Containers/4UEV752JH4.com.commonwealthrecordings.Vibe/`: `state.plist`, `artwork-<key>.jpg` and `waveform-<key>-{played,unplayed}[-light].png` once a widget is placed and a track plays, `placeholder-{dark,light}.png` from the theme, and the `widget-present` mark. The widget needs macOS 26, and placing one is a human step (desktop → Edit Widgets); `dump_state.widget.placed` reads the same gate as on iOS.

**TRAP: every registered copy of `Vibe.app` competes, and a stale one fails silently in three different ways.** A dev Mac holds many — `/Applications`, `build/DerivedData`, `build/AnalyzeDD` (every `make analyze`), `build/release`, other worktrees, archives. (1) **Buttons:** the system resolves the intents' host by bundle id and prefers `/Applications`; an installed build without the intents fails every click with `There is no metadata for VibePlayPauseIntent in com.commonwealthrecordings.Vibe` (chronod's log), whatever you just built. Unique build numbers and `lsregister -f` do not override it — only the build under test *being* the `/Applications` copy does (move the installed one aside, `ditto` yours in, restore after). (2) **Container reads:** the extension process can be an ad-hoc copy (`build/AnalyzeDD`'s, typically) that the container policy denies — `System Policy: VibeWidget deny(1) file-read-data …/state.plist` in the kernel log, and `codesign -dv <pid>` shows `TeamIdentifier=not set`. (3) **Rendering:** chronod keeps an extension process or path from a copy since replaced, and either rejects its output (`Bundle version did not match`, the medium tile falls back to grey placeholder bars) or the relaunch dies with `Invalid bundle record for current process`. Check `ps -o command -p $(pgrep -f VibeWidget.appex)` before believing anything the widget shows; `lsregister -u` the stale copies, kill the extension, and `killall chronod` (it relaunches) if the host still points elsewhere.

TRAP: **replacing the `/Applications` copy does two more silent things to a placed widget.** `lsregister -u` on it, even for the moment a reinstall takes, unregisters the extension and the desktop **drops the placed widgets**; swap the bundle in place and `lsregister -f` it, never `-u` first. And a new extension build draws nothing new by itself: the desktop keeps the last render, which survives `killall chronod` and killing the extension, until something reloads the timeline. Launching the app is the reload (its first publish), so a view change is only verified after the app has run once against the new build.

TRAP: switching signing identity on an existing build re-signs the extension but does not re-embed it, and the build fails `ValidateEmbeddedBinary` ("not signed with the same certificate"). Delete `Vibe.app/Contents/PlugIns` from the products and rebuild.
