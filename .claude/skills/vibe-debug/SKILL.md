...
`set_equalizer_mode activity|spectrum` is the same session-only A/B command on both apps. `spectrum` is the launch default: it converts each band's integrated energy to energy per octave, averages it across every complete FFT window in the callback, advances one strongest-band reference once for the full analyzed duration, then normalizes the coherent five-band average. It is a callback-time average, not an instantaneous spectrum. `activity` instead updates five private references per window and retains each band's largest normalized level. Success returns `ok:true`, the selected canonical `normalizationMode`, and `requested`, `tapObject` and `installed`; the last three make a failed active-tap replacement visible. Any other grammar returns the usage error. Changing an active mode synchronously replaces the tap, invalidates the old publication and resets the analyzer's partial window and reference history. Wait for `dump_levels.published` and `sequence` to advance before judging the new mode; counters are cumulative and do not reset. The selected mode survives later demand and engine-graph tap reinstalls in that process, while each replacement starts fresh analyzer history; relaunch restores `spectrum`. This is a comparison between Vibe's own two mappings, not a claim that either reproduces Apple's unpublished visualizer internals.
...
"$S" dump_levels         # live producer/renderer snapshot; schema and counter interpretation are in Equalizer counters above
"$S" set_equalizer_mode activity # A/B relative-activity normalization; use spectrum to restore the launch default
...
"$V" --debug-cmd dump_levels         # live producer/renderer snapshot; schema and counter interpretation are in Equalizer counters above
"$V" --debug-cmd set_equalizer_mode activity # A/B relative-activity normalization; use spectrum to restore the launch default
...
"$V" --debug-cmd dump_cloud_health     # {cloudParsesPending, cloudLaneHeld} — the metadata cloud lane at rest, on BOTH platforms. Both belong at zero once a sweep has settled; a held lane at rest is the whole sweep suspended
