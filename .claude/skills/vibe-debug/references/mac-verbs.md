...
Beta builds (`VIBE_VERBOSE_LOGGING=1`) automatically collect post-mix summaries on starts/seeks/resumes, gapless boundaries and late tap installation, bounded to three seconds. The player queue checks every 100 ms and logs on first signal; tap removal/replacement logs any partial capture immediately, retaining the filename, original play/voice and start reason; empty superseded captures and per-arm chatter are omitted. On hardware, the beta probe holds the meter for its bounded capture independently of visible equalizer demand; manual rendering needs ordinary meter demand. The probe reads no file. `Signal:` includes sample/host timestamps for the first sample above −60 dBFS, observed leading silence in milliseconds, peak, RMS and nonfinite count through the first signal buffer or interruption/expiry; late tap installation can miss the start, and it does not measure audible DAC output. Buffer timestamps exclude audio before the start and before outgoing fades actually settle. `observationStartMS` exposes the unmeasured prefix; `observedLeadingSilenceMS` counts only inspected silent frames, while `firstSignalAfterStartMS` measures elapsed time from the start to the threshold crossing (−1 if none). Without a threshold crossing, the silence figure is a lower bound; discarded overlap is never counted as silence. `--silent` zeroes buffers after the meter, so both silent HAL playback and the default pump launch exercise this signal measurement. Paired `Phase:` logs locate blocking HAL/engine work, and `Timeline:` joins queue admission, scheduling, callback delivery and the first advancing UI position. First-render logs separately label the host-clock estimate and poll observation delay.
