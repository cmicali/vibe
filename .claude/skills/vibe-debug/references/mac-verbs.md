...
"$V" --debug-cmd set_widget_placed off  # {ok, placed} — the publisher's gate as WidgetKit's answer (off) or the read signal (on) moves it; off stages the last widget going, which leaves the empty snapshot, without touching the desktop. The next activation's query sets it back to the truth
"$V" --debug-cmd widget_seek 0.5     # {ok, fraction} — the widget strip's click through VibeWidgetPerformAction, aimed at the current track: the launch waiter and the held seek included. With hang_open, a seek during an open (and, with no tags either, one held until the open lands)
