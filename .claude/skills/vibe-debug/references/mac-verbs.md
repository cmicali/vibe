...
"$V" --debug-cmd dump_audio_path     # {stages: [{stage, present, …}]} — the render chain from the source file to the output device, one entry per stage (source, decode, bus, varispeed, fx, meter, output, device): rates, sample formats, channels, whether each is in the render, the decoder's conversion (algorithm, quality, mixed, resampled), the varispeed's engagement and latency, the FX stages' activity. What Settings > Advanced's Audio group lists, raw
