...
The exclusive-access half cannot go through a loopback. Enable **Settings > Audio > Exclusive output** (off by default), with bit-perfect output on and a device selected that exposes writable hog mode and is **not** the macOS system output (physical and virtual devices are eligible). Disable Exclusive output for loopback capture. The channel can drive the switch with `settings_click "Exclusive output" on`. Check it against that device with Appendix A of `docs/future/bit-perfect-output.md` (a read-only HAL probe printing the hog owner's pid): `pid` = Vibe's while playing, `-1` from about 6 s after a pause, and `-1` with the device's format put back after `quit`.
