...
"$V" --debug-cmd block_render 2      # {ok, heldSeconds} — acknowledges immediately; the next real IO callback spins inside the pipeline for 0<seconds<=5 (debugHoldRenderInside:), so the render clock stops: the beta `Stall:` onset line, the audio IO thread's stack and the recovery line follow, `renderMaxMicros` records the cycle, and the HAL logs the overload. Audible as a gap; use --silent
"$V" --debug-cmd dump_now_playing_artwork # {artwork: art|thumbnail|placeholder|other|null, placeholderSide?: dark|light|both, placeholderReference?, windowAppearance} — which image the published artwork came from, by identity; a two-sided placeholder follows the window's appearance, not the system's
