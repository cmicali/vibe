...
Beta builds (`VIBE_VERBOSE_LOGGING=1`) automatically collect post-mix summaries on starts/seeks/resumes and late tap installation, bounded to three seconds. The player queue checks every 100 ms and logs on first signal; tap removal/replacement logs any partial capture immediately, retaining the original play/segment and start reason. Keep the playlist equalizer visible and active; no tap means an explicit unavailable line. The probe does not add a tap or file read. `Signal:` includes sample/host timestamps for the first sample above −60 dBFS, observed leading silence in milliseconds, peak, RMS and nonfinite count through the first signal buffer or interruption/expiry; late tap installation can miss the start, and it does not measure audible DAC output. Without a threshold crossing, the silence figure is a lower bound. `--silent` yields zero; `--no-audio-hw` without `--silent` exercises the software measurement without sound. Paired `Phase:` logs locate blocking HAL/engine work, and `Timeline:` joins queue admission, scheduling, callback delivery and the first advancing UI position. First-render logs separately label the host-clock estimate and poll observation delay.
