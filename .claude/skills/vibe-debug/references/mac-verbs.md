...
Beta builds (`VIBE_VERBOSE_LOGGING=1`) automatically collect three-second post-mix summaries on starts/seeks/resumes and late tap installation. Keep the playlist equalizer visible and active; no tap means an explicit unavailable line. The probe does not add a tap or file read. `Signal:` includes sample/host timestamps for the first sample above −60 dBFS, observed leading silence in milliseconds, peak, RMS and nonfinite count; late tap installation can miss the start, and it does not measure audible DAC output. `--silent` yields zero; `--no-audio-hw` without `--silent` exercises the software measurement without sound. Paired `Phase:` logs locate blocking HAL/engine work, and `Timeline:` joins queue admission, scheduling, callback delivery and the first advancing UI position. First-render logs separately label the host-clock estimate and poll observation delay.
