...
**The mac app embeds the desktop widget too** (`Contents/PlugIns/VibeWidget.appex`), and both mac paths check it after export with `asc-build-lib.sh`'s `asc_require_mac_widget` — `release.sh` on each exported app, `release-appstore.sh` on the expanded `.pkg`: the extension and the WidgetKit bridge bundle present (on the same slices as the app, for `release.sh`), the extension sandboxed, the group on both bundles, and no WidgetKit or SwiftUI linked into the app. The mac group is `4UEV752JH4.com.commonwealthrecordings.Vibe`, team-prefixed so neither path needs a profile or a portal capability for it (`VibeWidgetState.m`'s TRAP). The export re-signs the extension inside-out; nothing calls `codesign` on it by hand.
