...
**The mac app embeds the desktop widget too** (`Contents/PlugIns/VibeWidget.appex`), and both mac paths check it after export: `release.sh`'s `require_widget_extension` (present, same slices as the app, hardened runtime, sandbox, the group on both bundles) and `release-appstore.sh`'s mac postflight on the expanded `.pkg`. The mac group is `4UEV752JH4.com.commonwealthrecordings.Vibe`, team-prefixed so neither path needs a profile or a portal capability for it (`VibeWidgetState.m`'s TRAP). The export re-signs the extension inside-out; nothing calls `codesign` on it by hand.
