...
**ASan's malloc-zone statistics count freed allocations too.** A standalone 30,000-open `AVAudioFile` probe reported 139 GB in the ASan zone's `size_in_use` against 0.5 MB from `__sanitizer_get_current_allocated_bytes`. The health oracle uses ASan's allocator API for that zone, keeping the ordinary zone counters for CoreAudio's separate allocators. Otherwise every ASan soak eventually reports a fictitious leak.
...
**Reorder drags are in the op mix.** `reorder_begin` opens a synthetic row-reorder drag and deliberately leaves it live, so whatever the scheduler deals next — a replacing open, a removal, a convert, a burst — lands inside the session; `reorder_finish` later probes a slot and drops or cancels. That interleaving is the mid-drag race family (stale-drop rejection, a dragged row departing) that no pointer can stage, and a finish with no session live is a tolerated refusal that keeps the guard exercised. Reorder undo registrations also feed the `undo` op's stack.
...
`--profile theme` drives the theme record end to end — the store, the sanitizer and the apply. A theme apply is the app's widest single settings edit: every themed field repopulated at once, then `ThemeApply` re-resolving the window chrome, all four font slots, the playlist's colours and the waveform's palette in one turn. So what the profile weights is not the number of applies but **what is in flight underneath them** — opens stay heavy, because the artwork colour feeding the waveform palette rides the generation-matched install path, and an apply landing between an artwork delivery and its install is the case the colour-ownership guarantee is written for. Appearance flips are heavy for the neighbouring reason: every theme rule branches on dark, and a single-mode theme outranks `windowAppearanceStyle` outright, so the two only disagree when both move.

Its `theme_import` op is a **mutation** fuzzer, not a generator, and that distinction is the whole point: a record built from scratch is refused at the JSON reader, which never reaches the sanitizer — and the sanitizer is one gate over four callers (a JSON import, a stored record, a UI edit, and the shipped built-ins), so a value that escapes it escapes for all four. It starts from a real dumped record and corrupts one to four fields with absurd numbers, wrong types, malformed colours and nonexistent font faces. **The nonexistent font face matters most, because it sanitizes CLEAN** — a face name cannot be checked against installed fonts without asking for it — so it is the one hostile value that reaches the text system. Imports are capped (`MAX_THEME_IMPORTS`) because every accepted record is a *persisted* user theme.

`--profile playlist` drives structural edits under enough transport to make them dangerous. The model half deliberately cannot make the transport decision — removing the *current* row through the playlist alone would leave the player sounding an object the list no longer holds — so what the profile actually drives is the shell funnel owning the unload, the successor re-prefetch, the replacement play and the undo registration, none of which has an interesting branch unless something is playing. Opens stay in the mix rather than being zeroed: a replacement playlist is what makes a registered undo stale, and its restore must then die quietly instead of reinserting into a list it no longer describes — reachable only by editing, then opening, then undoing.

**Selection is reachable only because modified keys fall through.** `TransportKeyMonitor` handles bare Space, arrows, Return and both Delete keys itself, and passes anything carrying a modifier on to the focused table — so `key down shift` is `moveDownAndModifySelection:` and is the only way the channel can build a multi-row selection at all. A click into the pane first is what gives the table focus and an anchor. Both need the pane *open*, which is why `op_window`'s `toggle_size` emits as an off/on **pair** under these two profiles, for the same reason and in the same shape as `folder_art`'s: a uniform toggle parks the pane closed for half the run, and with it closed every row op is a no-op against the header while the summary still reads as a clean pass.

Row reorder is a real `drag`, not a model call: synthetic mouse events do drive `NSTableView`'s internal drag session, but **only with the window already key and enough interpolation steps** to clear AppKit's threshold — a drag posted into a non-key window silently does nothing, and a drop within one row of the source is a documented no-op rather than a failure.

**There is no `waveform_style` op any more, and its absence is deliberate.** The style used to be its own View submenu and is now a theme field, so every `click_menu waveform_style_*` answers "no menu item" — which the tolerated-error list swallowed, scoring a clean run over an op that had quietly stopped doing anything. Its weight went to `theme` in every profile, which swaps the renderer strategy anyway: the three built-ins carry three different styles and the import fuzzer sets the field directly.
