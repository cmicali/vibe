...
`outputDropouts` (macOS) counts the IO cycles the hosted output unit wrote as silence because the render returned an error; it is cumulative, so any growth is a finding and a soak holds it at zero. It, the render cost counters and `renderRefusals` are the ones `clear_render_counters` zeroes, for a measurement read per phase rather than as a delta. `renderRefusals` counts the renders the pipeline turned away because another was still inside — a carrier's callback stuck past its bounded stop while the next carrier began — cumulative and zero likewise, and `check_consistency` reports any (`graph.no_render_refused`). `renderCycles`, `renderMeanMicros` and `renderMaxMicros` (macOS) are the unit's callback cost over the cycles it rendered, cumulative too: diff the cycles and the mean across a run, and read the max against the device's buffer period, since a max that approaches it is the dropout before it happens. `retiredFades` is reported under `pending`, beside `hostedUnits` under `app` because they fail apart: a voice that never reports its end strands a fade entry while the unit count stays flat, and a unit count that moves by more than the varispeed (present in ordinary mode, absent in bit-perfect) means a unit was hosted without its predecessor being disposed. Both come from one `dispatch_sync` onto the player queue.
