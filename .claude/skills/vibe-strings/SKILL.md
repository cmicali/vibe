...
`Resources/InfoPlist.xcstrings` carries the bundle name, the copyright, and the `CFBundleTypeName` values. Its keys are plist keys, EXCEPT the document-type names, which are keyed on the **English type name** — that is how Launch Services looks them up, and it does work (`lsregister -dump` shows the localized names). It is covered by `make check-translations` on the same terms as `Localizable.xcstrings`, which matters because its failure mode is even quieter: a missing type name shows up only in the Finder's Get Info and Open With, nowhere in the app. `CFBundleName` is added to this catalog by Xcode's build, not by hand: it is kept with the same value in every language so the entry stays stable instead of reappearing as an untranslated `new` unit on the next build. The copyright reaches the About window only through `objectForInfoDictionaryKey:`; plain `infoDictionary[…]` does NOT apply `InfoPlist.strings`.
...
**The exception is a platform's FIRST version, which takes no release notes at all.** ASC refuses the field outright (`Attribute 'whatsNew' cannot be edited at this time`), so the uploader omits it and says so. The file still has to exist for the other platform, which is the normal case under Universal Purchase: iOS 1.12 was a first version while macOS 1.12 — the same version string — had a train behind it and needed its notes as usual.

**Screenshot captions are the one place a translation can fail the build.** They are burned into the image, so a caption that will not fit shrinks toward 72% of nominal and then errors out rather than rendering unreadably small. macOS captions are a headline plus a subhead; iOS is headline-only at 1.9x nominal, which makes it much the tighter of the two — expect some languages to need a looser translation rather than a literal one, and note the iPhone canvas binds before the iPad. `appstore-validate-copy` measures whether text **fits**, not whether it **breaks well**: ja/zh wrap at any character (kinsoku deliberately unimplemented), so a caption can split a word across lines and still validate. Three did on 1.12, splitting あ/ります and 播/放. Look at the rendered CJK screenshots and shorten until each sits on one line.
