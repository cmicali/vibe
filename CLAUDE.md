...
- **`VibeWidget/`** — the widget extension's own views and timeline, built once per platform (`VibeWidget`, `VibeWidgetMac`). Outside `Vibe/` for the reason `Tests/` is: nothing an app target recurses may hold it. Its contract with the app is `System/CLAUDE.md`'s.
