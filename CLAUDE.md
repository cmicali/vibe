...
- **The open the user is waiting on outranks every background read that would download a file.** On a file-provider folder the scarce thing is the provider's transfer. The rule lives in one object: `AudioFileMaterializationCoordinator` derives "a foreground transfer is active" from its own claim table (any live claim with a playback or prefetch waiter), yields metadata-only dataless claims at a foreground claim's registration — the sweep's in-flight download is cancelled, not waited out — and reopens admission at the settlement every path already funnels through, so no external release edge exists to be missed or doubled, on any platform. **A file already local is exempt throughout** — the rule and the transfer lanes bound provider *downloads*, which a local file never starts — so local tags and art keep landing while a cloud open has the wire to itself. Each shell also **defers** the sweep's start until the picked track's open settles (`scheduleDeferredMetadataLoad`, both platforms), which is a scheduling nicety on top of the rule and not the rule. The coordinator's half is `Audio/CLAUDE.md`'s; how the sweep asks `isForegroundTransferActive`, retries and re-ranks against it is `Audio/Metadata/CLAUDE.md`'s.
