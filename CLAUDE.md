...
- **The waveform theme always beats the style's built-in palette, and is resolved in one place.** The style setting is the geometry, the theme the colors; `WaveformTheme` (`Vibe/WaveformUI/`) is the only home of the identifier-to-colors rules, and renderers derive their gradient alphas from its hues, which is what keeps the default White theme pixel-identical to the pre-theme monochrome look. Each view resolves it — settings + appearance + on macOS the settled artwork color — and re-resolves on an appearance flip, because every rule branches on dark. Sonic Cirrus keeps its orange across the split by a one-time migration: a stored `sonic_cirrus` style with no theme key gets `orange` written at init (`SettingsRules.h`). The `album_art` color rides the artwork install path, which is already generation- and target-matched, so a stale art delivery can never color the wrong track's waveform; iOS has no artwork extraction yet, so there the identifier resolves to White's answer.
