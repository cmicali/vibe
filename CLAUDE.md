...
- **Editing a playlist row's existence is a model edit plus a shell transport decision, and the model half deliberately cannot make the second.** `Playlist.removeTrackAtIndex:` and its undo inverse `insertTrack:atIndex:` move rows, rebuild both indexes and touch no audio — removing the *current* row through the model alone would leave the player sounding an object the playlist no longer contains. Every gesture funnels the exact `AudioTrack` through `MainPlayerController.removePlaylistTrack:`, which owns the consequences: the one-row unload via `closeFile:`, the successor re-prefetch through `successorPrefetchTrack` (the same single prefetch funnel the On-track-end bullet above depends on), the replacement play carrying the ordered intent, and the generation-stamped undo registration whose restore dies quietly once the playlist it edited is replaced. **The file is never touched.** One structural edit sends **one** observer event; the mac reconciles it once, and iOS implements both observer methods as no-ops because it exposes no transport-safe removal path. The landing rules, race fallbacks and undo contract are `Mac/MainWindow/CLAUDE.md`'s; the cursor and index rules are `Vibe/Playlist/CLAUDE.md`'s.
