...
- **The equalizer bars follow the audio, from one demand-driven analyzer fed the final output samples inside the render.** The meter is the last stage of `VibeMasterBusRender` on both platforms — after the FX returns have re-entered, before the debug `--silent` zero — so it reads what reaches the device; `applyLevelTapOnQueue` installs it while wanted. The analyzer combines channels by power, caps its top band at 20 kHz and publishes sequenced five-band snapshots at a bounded rate. Each shell hands its rows an `EqualizerLevelSource`; a nil source is inactive, never synthetic animation.
