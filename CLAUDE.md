...
- **The 128px thumbnail is for list rows.** Both platforms archive compact bytes per row and decode visible thumbnails through `AudioTrackArtwork`'s shared bounded cache — the mac playlist's art cells, the iOS library rows, the iOS mini player. The iOS now-playing *page* deliberately draws none of it: it shows the full-size decode with nothing standing in for it. The mac header sits between the two: it draws the ≤640px display-art rendition archived beside the metadata entry (macOS-only, installed at one place like folder art — `Audio/Metadata/CLAUDE.md`), so a track change never re-reads the audio file for art.
