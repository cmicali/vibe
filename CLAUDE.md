...
- **`Vibe/Controls/`** — controls both platforms draw (`EqualizerIndicatorView`, whose bars follow the audio when handed an `EqualizerLevelSource` and run canned keyframes otherwise). A control earns a place here only when both apps draw the *same* one.
...
- **The equalizer bars follow the audio on both platforms, from one FFT tapped at whichever node feeds the output.** That node is not the same on each: with no FX segment — `enableFX:NO`, the iOS player — `mainMixerNode` is it, while with the segment present the reverb and delay returns re-enter *downstream* of that mixer, so the signal reaching the speaker is `AudioFX.masterBusOutputNode`. `applyLevelTapOnQueue` picks, which is also why macOS with FX switched off in Settings lands on the iOS wiring. Each shell hands its rows an `EqualizerLevelSource` (`PlaybackController` on iOS, `MainPlayerController` on macOS); a nil source falls back to the canned keyframes.
- **The tap runs only while an indicator is reading it, and the indicators say so.** `equalizerLevelsWanted:` is declared as an indicator starts and stops its display link, each shell counts consumers, and the FFT is off at zero — so a playing row scrolled out of the list costs nothing. Demand cannot see being *covered*, though, so each shell adds the condition only it knows: iOS the card over the library (`levelsOccluded`), macOS the compact size that hides the playlist by shrinking the window (`isPlaylistShown`), plus its window occlusion gate.
...
- **The open the user is waiting on outranks every background read that would download a file.** On a file-provider folder the scarce thing is the provider's transfer. Each shell **defers** the metadata sweep until the picked track's open settles (`scheduleDeferredMetadataLoad`, both platforms), and from play submission until that open settles the cache's **cloud lane is held** (`setCloudParsesHeld:`) — acquired synchronously in the player's pre-submit delegate edge (`willSubmitPlayForTrack:`) and released exactly once by the matching settlement: the successor prefetch's claim acknowledgement, the error path, or Close. The rule and its lanes are `Audio/Metadata/CLAUDE.md`; the call sites are each shell's `+PlayerEvents`.
