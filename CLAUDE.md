...
- **`Vibe/System/`** — bridges to OS services: Now Playing and remote commands, the widget's publisher and intents, cloud download progress, cloud file materialization. Both platforms drive them.
- **`VibeWidget/`** — the widget extension's own views and timeline, built once per platform (`VibeWidget`, `VibeWidgetMac`). Outside `Vibe/` for the reason `Tests/` is: nothing an app target recurses may hold it. Its contract with the app is `System/CLAUDE.md`'s. **`VibeWidgetCenter/`** is the app's one WidgetKit caller: compiled into the iOS app, and on the mac into a bundle loaded only once a widget exists, so the mac app never links WidgetKit (same doc).
