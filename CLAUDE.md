...
- **The reactive equalizer bars are iOS-only, and switched off the same way, inverted.** `AudioLevelTap` builds for both targets, but only the iOS shell hands its `PlaybackController` to a row's `EqualizerIndicatorView` as an `EqualizerLevelSource`; a nil source leaves the mac's keyframe path bit-for-bit unchanged. It is not mere scope: the tap sits on `mainMixerNode` bus 0, which is the signal reaching the speaker only while the FX segment is absent — with FX on, the reverb and delay returns re-enter downstream of that mixer and the same tap would miss every wet tail.
