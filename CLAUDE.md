...
`LogError`, `LogWarn`, `LogInfo`, `LogDebug` in `Vibe-Prefix.pch` wrap `os_log` under `com.commonwealthrecordings.Vibe`. **`VIBE_VERBOSE_LOGGING` (`project.yml`) decides whether info and debug are kept**: at 1, the beta setting, every level is written at Default, so Settings > Advanced > Save Debug Info and `log show` retrieve it after the fact; at 0, the stable-release setting, info and debug are not persisted and must be streamed live — see the `vibe-debug` skill. **The same flag compiles in the beta instrumentation**, all of it logging only: per-play `Timeline:` lines (input delay, submission identity, queue admission, open settlement and first observed node-clock progress; rendered progress does not measure audible output), a `HAL:` event log of every watched property change on every output device plus coreaudiod restarts and the alert output (`AudioDeviceManager`), `Callback:` lines when each system callback Vibe registers fires (HAL default/device-list, the output unit's device, bit-perfect's device listener, AVAudioEngine configuration changes, node segment completions, remote commands, sleep and wake), a `Recovery:` line with what every engine recovery saw and decided, `Stall:` lines when the main thread or the player queue cannot run anything for over 200 ms or the output's render clock stops while playing, at onset and recovery, with a `Stall stack:` of where the main thread is stuck (`AudioPlayer.m`; Vibe's frames print as offsets, so each beta's dSYM must be kept), a `Preflight:` line naming why a file was refused (`NSURL+AudioOpen`), and the M key's `USER MARK` (`Mac/MainWindow/Transport/`).

Beta `Phase:` begin/end pairs time output preparation, device pinning, format confirmation, exclusive ownership and engine/node start; `Timeline:` also includes seek/resume admission, main-thread callback delivery and the first displayed position. The optional `--diagnose-output-signal` flag records bounded `Signal:` summaries from the existing demand-driven post-mix tap, without installing an extra tap or reading a file. Save Debug Info bounds each optional fresh player/HAL section to two seconds, marks cached/unavailable results, and collects the persisted log independently.
