...
- **The fan-out is already the right shape.** `PlaybackController` broadcasts to an ordered weak `NSPointerArray` of `PlaybackObserver`s on main precisely because three views describe the same playback at once. The CarPlay list and now-playing templates become a fourth observer and need no new plumbing.
