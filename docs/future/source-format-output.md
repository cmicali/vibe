...
Keep the internal buffers planar initially. Use byte-sized ring storage and format-aware buffer access; select the sample operation outside the inner loop. A unity-gain, sole audible voice copies at most two ring spans straight into the output. It does not add to a zeroed float buffer or run through a converter. This also preserves signed zero on a float copy path.
