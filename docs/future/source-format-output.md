...
Audit the full ASBD: rate, integer/float representation, valid bits, signedness, packing/alignment, byte order, bytes per frame/packet, interleaving, channel count and channel layout. Extend `VibeFormatsMatch` where the current common-format comparison loses those distinctions. Never compare raw ASBD padding/reserved bytes.
