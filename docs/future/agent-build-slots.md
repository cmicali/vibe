...
- `xcodegen generate` mutex — `mkdir`-based lock (macOS has no `flock(1)`) around the generate
  ...
  `Vibe.xcodeproj` and the generated `Vibe/Common/Info.plist`, so a torn read is possible on a
...
- `Vibe/Common/AppDelegate.m:132-135` (`logBuildInfo`) — log the bundle id, making every log
