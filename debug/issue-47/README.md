...
## Beta10 file (c59): the beep test

**He heard the test file's first beep instantly with Exclusive output on** (c58). A file whose sound starts at 0 ms comes out of the A300 with no perceptible lag, so neither Vibe nor exclusive mode nor the DAC adds one.

**His session (c59, `tools/summarize.py`) agrees.** 67 track changes in 3 minutes, 60 with Exclusive on, three of them the test file. Click to first rendered frame was ~90–100 ms every time: input 1–7 ms, settle 2–27, node play 14–27, render 58. Settlement with Exclusive on now has a median of 8 ms against beta9's ~28 (the cached volume reads; the remaining report reads still take 2–4 ms each on his USB A300). **What varies is the music:** his MP3s open with 35–886 ms of silence (median 154): Message To The World 886, Choose Your Fate 455, Angel In The Swamp 436, Cannonball 225, See Through 157; the same tracks measure the same with Exclusive off. The 35 ms floor is Vibe's own 10 ms declick plus the player node's volume smoothing, which delay the −60 dBFS crossing even on the test file. Click to first sound: median 240 ms, max 983. Still no USER MARK. The only stalls were his Save click (609 ms, symbolicated to `saveDebugInfo:` waiting in `NSSavePanel init`) and a 300 ms one just before he switched Exclusive off, which ended before the first stack sample.

## Beta9 file (c54) and the beta10 changes

**His beta9 session (c54, `attachments/c54-*`, one 95 s run saved twice) confirms 16-bit and shows no Mac-side delay.** The A300 stayed at 44.1 kHz i16 throughout. He cycled Bit-perfect + Exclusive, Bit-perfect only, and Bit-perfect off, with 24 track changes: click to first rendered frame was 76–108 ms (median 97) with bit-perfect on and 72–78 ms (median 76) with it off. No `USER MARK`, no player-queue or output-render stall; five main-thread stalls (267–629 ms) all while opening Settings or the save panel. The comment does not say whether he still hears the lag.

**Apple's MP3 decoder already produces 16-bit samples.** His own log shows `ACMP3Decoder.cpp:157 Output format: 2 ch, 44100 Hz, Int16, interleaved`, and every MP3 measured here (test fixtures and five real 320 kbps files) decodes onto the 16-bit grid exactly, 100% of samples, 0.0000 LSB off. So beta8's 24-bit output carried those 16-bit values shifted left, and beta9's 16-bit output carries them unchanged: the same bits reached the DAC either way. AAC is different: it decodes to real float, only 0.25% of samples on the grid.

**The node, not the file, decides the decode format.** An `AVAudioFile` opened with `commonFormat:AVAudioPCMFormatInt16` still renders at float precision on a float-connected `AVAudioPlayerNode`, and a float file on an Int16-connected node rounds to 16-bit (`tools/node-decode-format.m`; `tools/decode-grid.m` measures the grid, `tools/device-formats.m` lists each output's formats; build with `clang -fobjc-arc -framework Foundation -framework AVFAudio -framework CoreAudio`). Beta10 therefore connects the node at Int16 for a lossy source when the device is prepared at 16-bit (`VibeBitPerfectDecodesAsInteger16`, `playerNodeFormatForFile:`), which is the literal "16-bit from the decoder" he asked for. Debug info shows it as `playerNodeFormat: 44100 Hz i16 interleaved 2ch`; the log says `bit-perfect: decoding X straight to 16-bit integers`.

**The rest of the bit-perfect path, verified.** The render suite already compares the engine's output with the decoded file at tolerance 0 in bit-perfect mode (rate/depth/channel matrix, MP3, MP2, lossless containers): no varispeed, no FX, the mixer at unity is a pass-through, and the master bus runs at the device rate. The one exception was the 10 ms declick fade-in at every start, seek and resume, which scaled those first samples; bit-perfect output now writes no volume at all, and the tests compare it from the first sample. The device's own volume, balance and mute are reported; his were 1.0, centered, unmuted.

**Bit-perfect costs his A300 ~20 ms per track change on the player queue, and a DisplayPort device here nothing.** Submit to settlement was 24–36 ms with bit-perfect on against 7–11 ms off; on this Mac's H27P3 (i16/i24 like the A300) it is 4–7 ms. No settlement parked. The likely cost is the report's device reads: every state publication and fade completion re-read volume, balance and mute, ten-odd HAL calls each. Beta10 caches those per prepared device while the wildcard output-scope listener is registered, and logs each fresh read and any report whose device reads take 2 ms or more (`bit-perfect: report read device ...`), which will confirm or refute this on his Mac.

**Other beta10 diagnostics.** Stall stacks are complete: the beta9 ones were cut by the log's ~1 KB message limit before any Vibe frame, and a 128-frame walk would have lost the bottom of any deeper stack. A sample now keeps the first and last 512 frames of any depth, collapses repeats, logs the top 40 and bottom 60 across numbered lines, and repeats every 500 ms while the stall lasts (six at most). The player queue is now sampled too, through the pool thread draining it, and a stalled render clock samples CoreAudio's IO thread. Verified with `block_main_deep` at depths 5, 600, 3000 and 300 alternating: every sample reaches `dyld start`, the longest line is 859 bytes, and `tools/symbolicate.sh <file> <dSYM>` names Vibe's frames down to file and line. The signal probe now holds the level tap itself for each start's capture, because his theme hides the number column the equalizer lives in, which is why all 41 beta9 captures said "no active level tap". A settlement that parks for a format switch logs the formats that forced it. The ~150 `HALDefaultDevice.cpp: Could not find default device` errors in his log come from CoreAudio itself (his Mac has no input device); Vibe never asks for one.

**The counter, and two tools for the next file.** At an ordinary song's 3 Hz the time label could turn 0:00 → 0:01 up to 333 ms after the second passed, which over a quiet intro looks like nothing happening; a start, resume or seek now aims one update just past that change (measured: the label turned at a position of 1.03–1.06 s, within the polling interval). `tools/summarize.py <debug-info>` prints one row per track change — input, settle, node play, first render, intro silence, input to first sound, first UI movement, exclusive — and each USER MARK against the play before it; on beta9's c54 it reproduces the hand analysis above. `tools/vibe-lag-test.mp3` (`make-lag-test.sh`) is 20 s at 320 kbps with a beep starting at 0 ms and on every second after (the first higher): CoreAudio's decode starts it at frame 1, so a lag he hears on it is not a file's leading silence.
