...
- The editor's sections run Name, Window (appearance, background, tint, corner radius), Fonts, Player (the four label colors, then the info-display rows), Waveform, Playlist (background style and colors, then the row fills). The **Appearance popup is the theme's `appearance` field** (Auto / Light / Dark): a pinned choice makes a fixed theme, and `resolveLayoutStateFromSettings` collapses every Dark/Light well pair to the pinned side's single well (captions hidden) and keeps only the pinned side's tint and custom-waveform rows — the off-side halves stay stored, so nothing is lost flipping back. The **corner-radius slider carries a live px readout and a magnetic detent** at the factory radius — dragging near `kVibeThemeCornerRadiusDefault` snaps onto it, the reset without a button.
