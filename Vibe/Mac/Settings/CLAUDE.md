...
- The editor's sections run Name, Window (background, tint, corner radius), Fonts, Player (the four label colors, then the info-display rows), Waveform, Playlist (background style and colors, then the row fills). The **corner-radius slider carries a live px readout and a magnetic detent** at the factory radius — dragging near `kVibeThemeCornerRadiusDefault` snaps onto it, the reset without a button.
