...
| `colorwell` | `#RRGGBB` | sets the well's color, then sends the action; the dump's `value` is the same hex, so the setting round-trips through this verb. The Appearance pane's two wells share their row label — address them as `#index` |
