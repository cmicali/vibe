...
- **A card of three rows** — Web, Support and GitHub — each a hyperlink-styled plain `NSButton` showing the URL trailing, so the debug walker can address them by row title or by URL. Support opens `kVibeSupportURL` (`Mac/App/AppDelegate.h`), the same address Help > Get Support uses.
