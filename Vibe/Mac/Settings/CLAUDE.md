...
- **The frame-tracking constants sit at 999**, just below required, so the one pass where the frame moved ahead of the constants stretches instead of breaking loudly; `windowDidResize:` lands the constants on the same turn.
...
The cost of those two is that a pane's own size change cannot move an open window either: at 999 the window edge wins, and the swallowed `preferredContentSize` tells AppKit nothing. So the shared-size pass tells the host instead — `SettingsPaneSizeHost`, which the tab controller answers by re-setting `contentMinSize` and growing the window to the new floor where needed, inside the pane-owned animation context — grow-only, so a user-enlarged window keeps its frame. Without it a revealed row would clip against the old frame.
