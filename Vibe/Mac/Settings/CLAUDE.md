...
## TRAP: the pane's fitting size is the window's FLOOR, and an actual floor change has exactly one animation path

The constraint engine re-sizes a `contentViewController` window to its content's fitting size after every layout pass (`_changeWindowFrameFromConstraintsIfNecessary`) — which is exactly why the pane constraints are **minimums**, not equalities: with an equality the engine snapped every user resize back, while a `>=` leaves the fitting answer at the floor and a larger user-held frame stable. The pane's 999 minimum constraints define the floor the window settles at when first shown (safe-area height + width, so the titlebar overlay is the engine's to add); the frame autosave restores whatever the user held, and the sidebar's fixed thickness plus divider joins the width on its own.
...
- **The pane constraints sit at 999**, just below required. A required minimum breaks loudly against a stale autosaved frame below the floor; at 999 the window edge wins for one pass — the pane compresses briefly — and the tab controller's grow-to-floor pass corrects it. The window edge winning is also what lets the pane stretch while a frame animates.
...
- **A built-in's page is read-only**: every editor control disables (the walker reports it honestly and `settings_click` refuses), the Name row hides, and the header carries the caption plus **Duplicate**, which mints an editable copy and re-renders editing it.
...
- **TRAP: the list's programmatic reload guards reentrancy** (`_refreshingThemeList`): `reloadData` and the reselect both post selection-changed, and treating those as user activations recursed refresh into a stack overflow. Observed, not hypothetical.
