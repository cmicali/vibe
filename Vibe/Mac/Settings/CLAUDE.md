...
## TRAP: the pane's fitting size IS the window's settled size, and an actual size change has exactly one animation path
...
The cost of those two is that a pane's own size change cannot move an open window either: at 999 the window edge wins, and the swallowed `preferredContentSize` tells AppKit nothing. So the shared-size pass tells the host instead — `SettingsPaneSizeHost`, which the tab controller answers by applying the selected pane's exact frame inside the pane-owned animation context. Without it a revealed row would clip against the old frame.
