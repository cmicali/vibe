...
The constraint engine re-sizes a `contentViewController` window to its content's fitting size after every layout pass (`_changeWindowFrameFromConstraintsIfNecessary`), and **no constraint arrangement on the panes survives it resizable**: equality constants snapped every user drag back to the constants, minimums snapped the drag back down to the floor, and no constraints at all snapped to the sidebar's own answer — all three observed. So the panes carry no size constraints, and `SettingsWindowController` pins the **content view equal to the live frame** instead: `windowDidResize:` copies every new size — the user's drag, the grow-to-floor pass, the autosave restore — into a constant pair at `NSLayoutPriorityDragThatCanResizeWindow - 1` (509), so the engine's fitting answer is always the frame the window already holds and the snap is a no-op. The floor lives solely in `contentMinSize`; the window seeds its first size from the settled shared size explicitly (nothing else can size it), and the frame autosave restores whatever the user held.
...
- **The frame-tracking constants sit at exactly 509, `NSLayoutPriorityDragThatCanResizeWindow - 1`, and both neighbors are observed failures.** A user's edge drag is evaluated at 510: at 999 the equalities beat it, the resizable range collapsed to a point, and AppKit showed no resize cursor at all — while `settings_resize`, which sets the frame directly, kept working, the trap that hid it. At `NSLayoutPriorityWindowSizeStayPut` (500) they lose the opposite fight: the fitting snap re-sizes the window to a content-derived answer and ignores the floor. `windowDidResize:` lands the constants on the same turn as any resize.
...
- The editor's sections run Name, Window (appearance, background, tint, corner radius), Fonts, Player (the four label colors, then the info-display rows), Waveform, Playlist (background style and colors, then the row fills). The editor's **Appearance popup is the theme's `mode`** (Light & Dark Modes / Single Mode) — the theme's COLORS, not the window's appearance, which is the list page's popup. Under Single Mode `resolveLayoutStateFromSettings` collapses every Dark/Light well pair to one well with captions hidden — the dark-keyed well, the single slot's home, which `AppTheme` reads and writes from either side — and keeps only the dark-keyed tint and custom-waveform rows. The light halves lie dormant, so flipping modes round-trips both palettes. Background and mode edits request `WindowAppearance` beside their color effects because a single-mode theme's solid background pins the window to its luminance side (`AppTheme.requiredWindowAppearance`); the three seeding loops start at the dark pass under single mode, or the light default would land in the single slot. The **corner-radius slider carries a live px readout and a magnetic detent** at the factory radius — dragging near `kVibeThemeCornerRadiusDefault` snaps onto it, the reset without a button.
- The custom tint, solid background and custom waveform wells are built always and hidden unless their popup says so, **seeded from their displayed fallbacks when chosen**, so the window and waveform immediately match what the wells show. Alpha is part of every choice (a wash's strength, a solid background's opacity, a waveform side's resting level). The **playlist has the same glass/solid choice as the window** — solid hides the frost outright, and the one playlist color pair is the wash over glass or the solid cover, seeded near-opaque on choosing Solid.
- In the debug channel the theme table is addressed by index — `settings_click '#2' <row>` — because the Themes card's header labels every control in it, the same shape as the Files pane's folder list. Assert activation through `dump_state.settings.activeTheme`.
