...
- The editor's sections run Name, Window (appearance, background, tint, corner radius), Fonts, Player (the four label colors, then the info-display rows), Waveform, Playlist (background style and colors, then the row fills). The **Appearance popup is the theme's `appearance` field** (Light & Dark / Always Light / Always Dark): a pinned choice makes a single-look theme, and `resolveLayoutStateFromSettings` collapses every Dark/Light well pair to the pinned side's single well (captions hidden) and keeps only the pinned side's tint and custom-waveform rows. The single well edits the theme's ONE color set — `AppTheme` mirrors the pairs while pinned — so flipping between Always Light and Always Dark carries the colors. The **corner-radius slider carries a live px readout and a magnetic detent** at the factory radius — dragging near `kVibeThemeCornerRadiusDefault` snaps onto it, the reset without a button.
