...
**`NSApp.helpMenu` is set explicitly** — that is what puts AppKit's Search field at the top in every language, rather than AppKit finding a menu titled "Help". The app ships no help book, so Get Support (`kVibeSupportURL`, `Common/VibeProductURLs.h`) is the only item of ours; the search field is inserted when the menu opens, so `dump_menu` on an unopened Help menu reports Get Support alone.
