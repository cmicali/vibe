...
**3. The playlist frost.** A behind-window `NSVisualEffectView`, `UnderWindowBackground` in both appearances (the light `WindowBackground` material is effectively opaque paint), plus the wash layer over it — the theme's playlist background color when set, else a brightening white wash in light mode only. Row text is not readable over Clear glass. It sits **under** the scroll view, since an `NSClipView` background does not composite semi-transparent colors over a backdrop, so it also covers the empty area below the last row. The theme's **solid** playlist style hides the frost outright — the wash layer then carries the whole background over whatever the window backdrop shows (`applyPlaylistBackground`).
