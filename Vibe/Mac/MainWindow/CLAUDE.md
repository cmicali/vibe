...
## Removing a playlist row

`removePlaylistTrack:` is the one funnel for every removal — the Edit menu, the two delete keys and the row context menu, whose request arrives through `PlaylistController.removeTrackRequestHandler`. It receives the exact `AudioTrack` and resolves its live row once, so a shifted object is followed and a departed one is ignored. **`Playlist.removeTrackAtIndex:` deliberately touches no audio**, so this is where the playback consequences are decided; removing the current row through the model alone would leave the player sounding a track the playlist no longer contains, which would make every correct player callback fail its identity guard and leave auto-advance with no authoritative successor. **The source file is never touched** — no delete, trash, rename or move.

The order is the contract:

1. **Resolve the exact object once** through `getIndexForTrack:`. A departed object no-ops; an object shifted by an earlier edit is removed from its live row.
2. **A one-row playlist is an unload, not an edit** — `closeFile:` and return, without mutating the model first. It owns the whole teardown (stop, download monitor, parked successor, waveform load, deferred metadata and the scan, error mask, UI timer, empty state) that a model-only removal would skip. There is deliberately no *second* whole-list command: File > Close already retitles itself to Close All Files.
3. **Resolve the ordered intent before mutating**: whether the row is current, whether a forward successor exists, and — only when both are true — `AudioPlayer.playingIntentAfterPendingCommands`. That one action-only queue barrier includes a play, pause or resume already submitted before the removal without introducing a second mirrored transport state. A last-row removal always parks, and a non-current removal leaves transport alone, so neither waits. `outputAudioActive` answers whether pixels and the FFT should run and is not a transport answer at all.
4. **Mutate, then refresh the metadata neighborhood once** from the final rows. The playlist's cursor callback is not raised for a structural edit, so this is the removal's own reconciliation rather than a second notification of one event.
...
**A removal is undoable as the list edit it is.** The funnel registers `reinsertPlaylistTrack:atIndex:generation:` on the window's one undo stack under the menu item's own action name. Undo restores the exact object to its row — selected and revealed, its metadata re-requested through `loadMetadataNow:` because removal abandoned the row's queued scan work — and deliberately not transport: a removed current row does not replay, exactly as removing it did not restart what survived. The registration is stamped with `PlaylistController.structureGeneration`, which follows the model's own replace-all announcement rather than any shell call site — every path that replaces or clears the list bumps it, present and future — so a restore whose playlist has been replaced or closed since dies quietly instead of editing the successor list; its redo then resolves a departed object and no-ops in the funnel's guard, and the dead pair falls off both stacks on its own. The window's undo manager is capped (`levelsOfUndo`) so dead registrations cannot pin removed tracks unboundedly. The sole-row removal routes to `closeFile:` before registering, so Close stays un-undoable. Also because of removal, the funnel's re-prefetch and the restore's are both gated on a not-Stopped player: a Stopped player advances nothing and holds no parked handle, so an errored player must not open — or download — a successor over a mere list edit.
