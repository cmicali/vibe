...
The bootstrap itself starts outside: `main.m` in the repo root, then `AppDelegate`, then `MainMenuBuilder` (`Vibe/Mac/Menu/`) and `MainPlayerController` (`Vibe/Mac/MainWindow/`). `Vibe/Mac/Menu/CLAUDE.md` covers the launch sequence.
...
- **`DocumentTypes`** is no longer here — it is `Vibe/Common/`, since it reads the bundle and touches no AppKit, and both targets declare document types. The `⌘O` panel's filter and `DefaultAppRegistration` (`Vibe/Mac/Settings/`) still read it, so the two cannot disagree about what a "supported file" is.
