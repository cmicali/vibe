...
Restoration resolves bookmarks off the main thread and posts its change notification **coalesced to one per run-loop turn**, because it settles one bookmark at a time and each observer does real work; the user-driven add and remove post directly, so the pane redraws in the same turn as the click.
