...
- **`removeTrackAtIndex:` and its inverse `insertTrack:atIndex:` are the mutations that MOVE rows**, so they rebuild both indexes (`rebuildIndexes`) instead of patching them — every entry below the edit names the wrong row otherwise. O(n) is what `NSMutableArray`'s own edit already costs, and an explicit user gesture can afford it; recreating `AudioTrack`s from URLs could not, since it would throw away installed metadata and artwork and defeat every identity check. The cursor follows the rows in both directions so it keeps naming the same object: a removal above it decrements it, an insert at or before it increments it, removing the current row lands on the successor that slid in or on the new last row, emptying the list resets it to 0, and inserting into an empty list leaves it at 0 naming the new row. Insert clamps a past-the-end index to the end — the undo of a removal can land after later edits have shortened the list, and it should restore the row as the last one rather than not at all.
- **The model does not touch the player.** Removing the *current* row through it alone would leave the player sounding a track the playlist no longer contains, so the calling shell owns that transition — on macOS `MainPlayerController`'s removal funnel (`Mac/MainWindow/CLAUDE.md`). iOS exposes no remove UI, and any future one must add its coordinator first rather than calling the model directly.
- Observers get `playlistDidReplaceAllTracks:`, `didAppendTracksAtIndexes:`, `didReplaceTrackAtIndex:`, `didRemoveTrackAtIndex:`, `didInsertTrackAtIndex:` and `currentIndexDidChangeFromIndex:`. There is **one** observer slot — the iOS shell's ordinary fan-out to several views is `PlaybackController`'s job, while its removal method stays a no-op until iOS has a transport-safe coordinator (`Vibe/iOS/CLAUDE.md`).
- **One structural edit sends one event.** The removal event carries only the removed index, which is all the table reconciliation consumes, and deliberately does *not* also fire `currentIndexDidChangeFromIndex:`: two callbacks for one edit make the mac table reconcile against the same action twice and obscure the ordering.
