...
## Reordering: the internal drag

Rows reorder by dragging within the table, and only there — the drag carries a **private pasteboard type** whose payload is a per-session token, never a file URL, so it cannot read as an Add/Replace open anywhere (`MainWindow` additionally refuses any drag with an in-app source). The authoritative payload is the controller's retained array of the exact dragged `AudioTrack`s: at **every** validation and at the drop the objects are re-resolved through `getIndexForTrack:`, so a row number captured at mouse-down is never trusted. A replaced playlist resolves every object to `-1` — zero survivors rejects the stale drag outright — while a single row swapped by a background conversion drops out alone and its companions still move. Token and array are cleared when the session ends, drop or cancel.
