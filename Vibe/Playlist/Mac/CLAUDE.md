...
**The observer reconciles each of remove, insert and move once — `removeRowsAtIndexes:`, `insertRowsAtIndexes:`, or the rules header's evolving `moveRowAtIndex:toIndex:` sequence inside `beginUpdates`/`endUpdates` — with no animation**: thousands of rows sliding is motion nobody asked for, and row views survive so the wash and the equalizer travel with their rows. **TRAP: a move has no animation argument, so it runs inside a zero-duration `NSAnimationContext` group.** AppKit's animated move left a blank slot for a row arriving from off screen, and a row still sliding when `endUpdates`' zero-delay cleanup ran stayed in the table's private row-view purgatory for good. The group closes the blank slot and most of the retention; rapid back-to-back reorders or undos can still strand a few row views, an AppKit bug with no public-API cure (row view reuse is kept deliberately — it is what large playlists need). Every edit then runs the same tail before returning to the run loop, so no frame shows two playing rows: `refreshRowViewPlayingStates`, then `reconfigureVisibleNumberCells`.
