...
The move observer applies the rules header's **evolving-coordinate sequence of `moveRowAtIndex:toIndex:` calls** — never `reloadData`, for removal's reasons: row views, the playing wash and the equalizer travel with their rows — then selects the landed block, scrolls its first row visible, and runs the same re-stamp/reconfigure pair as removal, all before returning to the run loop so no frame shows two playing rows. `playlistOrderDidChangeHandler` fires last: the shell's one follow-up edge (successor re-park, metadata neighborhood, transport UI), fired from the observer rather than the drop site so any future move initiator gets it for free. A move never enters a play funnel — the current object's identity survives, whatever moved.
