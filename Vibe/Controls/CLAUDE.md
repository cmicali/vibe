...
The **waveform style** is the hairline midline both waveform views draw; they own *when* to show it and how wide (`WaveformUI/CLAUDE.md`). Its traps are the control's own:

**TRAP: the band sweeps in from before its span and out past the end, and the host layer does not mask to bounds** — so it rides inside a `masksToBounds` clip layer (`_shimmerClip`) sized to that span. Without it the shimmer draws over the artwork on one side and out to the window edge on the other.
...
Each style therefore has exactly one indeterminate motion, both compositor-only, and the first fill sample retires the pulse. Style changes metrics, not structure: `LoadingIndicatorMath.h` is the per-style table and the one seam under test. **The waveform style's numbers are a regression fence** — pixel-identical to what the control drew before the row style existed.

`LoadingIndicatorView` is the row gutter's host, structured like `EqualizerIndicatorView` — `#if TARGET_OS_OSX` only for the superclass and the layout/appearance/backing hooks. It is much cheaper than the equalizer: the sweep is one repeating `CABasicAnimation` on one layer — no display link, no timer, no app-side per-frame callback, no path rebuild; the whole thing runs on the compositor. What it must not do is hold a live animation for a row nobody can see, so **`active` goes NO on cell reuse and on hiding** — the row wiring in `Playlist/Mac/` and `iOS/` owns those — **and the view drops it itself when it leaves its window** (`viewDidMoveToWindow`, `didMoveToWindow`), which is what scroll-out and detachment are; a row scrolled back in is re-configured by its table, so `active` comes back with it. `active = NO` tears everything down; `endSweepKeepingFill` is never used here (it is the iOS scrubber's "data landed mid-download" case, which has no row equivalent). The macOS empty-state line reads the waveform style's height and track alpha from the math header, so the empty line and the loading track cannot drift apart.
