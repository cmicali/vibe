...
- **`VibeManualRenderPump`** stands in for the HAL IO thread that `--no-audio-hw`'s manual rendering never starts, pulling frames through the engine at real-time pace, so voices are consumed and `lastRenderTime` advances as they would against hardware. `VibeAudioTests` uses the same pump with explicit frame pulls and a virtual clock: the production FX and idle callbacks run at sample-time deadlines, even while a stopped engine yields silence. The pump carries the two things the IO thread's cadence gives the player as hooks: `beforeRender`, before every slice, where the frame-driven mode decodes inline (the paced mode leaves decoding to the bus's own queue, as hardware does), and `afterRender`, after every slice, where the player drains the bus; `starveDecoder` skips the first, so a test can watch the bus underrun and recover. An optional capture block sees the actual PCM. Rebinding after an engine rebuild cancels the previous timer; zero-frame temporary render failures have a bounded retry, while partial/error results fail rather than silently lose samples. It is what lets the whole player run with no audio device, under a sanitizer or on a machine with none.
