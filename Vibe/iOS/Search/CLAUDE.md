...
- **A row is never added without its bookmark.** Minting needs the folder's scope open, which only `FolderSession` can promise (`bookmarkOpenFolderWithCompletion:`), so the star fills when the bookmark lands. `addFolderURL:bookmark:` dedupes by path.
