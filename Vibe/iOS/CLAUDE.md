...
The Files tab: `UIDocumentBrowserViewController`, the system's own browser over every Files provider. A picked file goes through `PlaybackController.openExternalURL:openInPlace:` — the same road the picker takes. It is the browser rather than an embedded `UIDocumentPickerViewController` because the picker is a modal presentation and unsupported as a child, and it takes **no navigation controller**: the browser brings its own bar and hierarchy, and wrapping it stacks two.
...
**The transport is always up**; only the empty state fades it (`chromeAlpha`). It used to hide while playing, on the theory that a screen tap was the pause control — the tap still is, everywhere off the waveform and the buttons, but it is no longer the only one.

**The waveform sits at the same y on every page, and that is a layout guarantee, not a coincidence.** Waveform, time row and transport are one chain off the *safe bottom*, and the portrait header labels are given exact line-count heights — two lines for the title, one each for artist and codec — so a two-line title, a missing artist or a shrunk-to-fit one cannot move anything below them. All three shrink to fit their width rather than truncate, because with the height reserved a long title has nowhere to grow. Only the art card floats, centered in the band left above the waveform, and it is still the thing that gives at accessibility text sizes.
