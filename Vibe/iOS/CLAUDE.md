...
- **`AudioSessionController`** — the AVAudioSession half of what `AudioPlayer+Devices` is on macOS. Playback category activated before every play (never at launch); interruption, route-loss, media-services-reset, and engine-config-change all become delegate pause/resume verdicts mapped onto the public transport API. The engine's own idle-stop → lazy-restart path is the recovery; nothing here touches the graph.
