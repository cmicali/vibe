...
The longest file in the tree, so: **[The model](#the-model)** is `PlaybackController`, everything the app plays and nothing that draws it, plus **[FolderSession](#foldersession)**, the picked location's owner. **[The shell](#the-shell)** is everything that draws, one heading per screen — `RootViewController` (the app's shape), `MiniPlayerView`, `FilesViewController`, `LibraryViewController`, the `Settings/` hierarchy, `SearchViewController` with `FileSearchIndex` and `SearchFolderStore` behind it, and `PlayerViewController` — the card — with `TrackPageCell` and `PageWaveformCoordinator`. **[Building and verifying](#building-and-verifying)** is the simulator loop and the debug channel's iOS half.
