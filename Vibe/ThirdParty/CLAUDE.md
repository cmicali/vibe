...
Everything here is other authors' code, vendored in-tree — there is no package manager, and CocoaPods is gone — and compiled as part of the app target: `taglib/`, `PINCache/` and `PINOperation/`. **Do not restyle it.** Adding or removing a file means editing `project.yml` and re-running `xcodegen generate`.
