...
Seven settings are read far more often than the rest and are cached in the class: `showRemainingTime`, `showFileInfo`, `showBPM`, `showKey`, `keyNotation`, `keyColorsEnabled`, `uiUpdateHzCap`. All seven are macOS settings, which is why the whole cache is inside the `#if`. Main thread only.
