...
//
// The window-layout constants shared by MainWindow, for frame and minimum and
// maximum sizing, and MainPlayerContentView, for the design-time layout, so
// that the two cannot drift apart. They are plain scalars, so static-in-header,
// which gives one private copy per importer, is fine.
