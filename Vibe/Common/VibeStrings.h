...
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_NEEDS_DEVICE NSLS(@"settings.general.exclusive_output.needs_device", @"Choose an output device other than system output", @"Settings: exclusive output requires an eligible device other than the system output device. Virtual devices are allowed. No trailing period.")
...
#define STR_SETTINGS_BIT_PERFECT_NEEDS_DEVICE  NSLS(@"settings.general.bit_perfect.needs_device",   @"Choose a wired device", @"Settings, Audio pane: caption under the disabled bit-perfect output switch while the output is System Output or a Bluetooth, AirPlay or other device the mode cannot drive. No trailing period.")
...
#define STR_SETTINGS_BIT_PERFECT_FORMAT_SHARED_DEFAULT NSLS(@"settings.general.bit_perfect.format_shared_default", @"%1$@ · %2$@-bit · shared (system output device)", @"The output format bit-perfect output settled on, on a device the app deliberately does not take exclusive use of because it is the system's output device, so other apps' sounds can still mix in: %1$@ is the sample rate, already formatted with its unit (e.g. '96.0 kHz'), %2$@ the word length in bits.")
