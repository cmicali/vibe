...
#define STR_MENU_APPEARANCE_DARK    NSLS(@"menu.view.appearance.dark",   @"Dark",               @"Appearance choice in Settings: always use the dark appearance.")
...
#define STR_SETTINGS_AUDIO_SECTION      NSLS(@"settings.general.audio_section", @"Audio Output",                          @"Settings window: the Audio pane sidebar item and title, for output device, bit-perfect output and exclusive output.")
...
#define STR_SETTINGS_OUTPUT_LABEL       NSLS(@"settings.general.output_label",  @"Output:",                        @"Settings, Audio pane: heading above the output device list. Ends with a colon, stripped at display.")
#define STR_SETTINGS_DEVICE_NAME       NSLS(@"settings.audio.device.name", @"Name", @"Settings, Audio pane: Header of the output device name column.")
#define STR_SETTINGS_DEVICE_TYPE       NSLS(@"settings.audio.device.type", @"Type", @"Settings, Audio pane: Header of the output device connection type column.")
#define STR_SETTINGS_DEVICE_BUILT_IN   NSLS(@"settings.audio.device.built_in", @"Built-in", @"Settings, Audio pane: Output device type: audio hardware built into the computer.")
#define STR_SETTINGS_DEVICE_AGGREGATE  NSLS(@"settings.audio.device.aggregate", @"Aggregate", @"Settings, Audio pane: Output device type: multiple audio devices combined into one aggregate device.")
#define STR_SETTINGS_DEVICE_VIRTUAL    NSLS(@"settings.audio.device.virtual", @"Virtual", @"Settings, Audio pane: Output device type: a software audio device.")
#define STR_SETTINGS_DEVICE_OTHER      NSLS(@"settings.audio.device.other", @"Other", @"Settings, Audio pane: Output device type: an unknown or other connection type.")
...
#define STR_SETTINGS_ALWAYS_ON_TOP      NSLS(@"settings.general.always_on_top", @"Keep window on top",                  @"Settings, General pane: checkbox that keeps the player window above all other apps' windows. The same setting as the View menu's 'Always on Top' item, in sentence case per checkbox convention.")
#define STR_SETTINGS_LOCK_WINDOW_POSITION NSLS(@"settings.general.lock_window_position", @"Lock window position",   @"Settings, General pane: switch that stops the player window from being moved by dragging it. The window can still be resized from its edges. Sentence case, like the other switches.")
...
#define STR_SETTINGS_REOPEN_PLAYLIST_CAPTION NSLS(@"settings.general.reopen_playlist.caption", @"Playlist is saved on quit and loaded on next launch", @"Settings, General pane: caption under the 'Reopen last playlist' switch. One sentence, no trailing period, saying when the playlist is saved and when it comes back.")

#define STR_SETTINGS_ON_END_LABEL        NSLS(@"settings.playback.on_end_label",                @"On track end",            @"Settings, Playback pane: label beside the dropdown choosing what happens when a track finishes playing.")
...
#define STR_SETTINGS_PITCH_RANGE_LABEL   NSLS(@"settings.playback.pitch_range_label",           @"Pitch range",             @"Settings, Playback pane: label beside the pitch fader range choice (8% or 16%).")
#define STR_SETTINGS_SKIP_STEPS_LABEL    NSLS(@"settings.playback.skip_steps_label",            @"Skip steps",              @"Settings, Playback pane: label beside the dropdown choosing how many bars the three skip actions jump.")
...
#define STR_SETTINGS_CROSSFADE_LABEL     NSLS(@"settings.playback.crossfade_label",             @"Crossfade",               @"Settings, Playback pane: label beside the dropdown choosing how long one track blends into the next on a track change.")
...
#define STR_SETTINGS_PLAYBACK_AUDIO_SECTION NSLS(@"settings.playback.audio_section",             @"Audio",                    @"iOS Settings, Playback screen: heading above the Resampling row, which sets the quality of sample-rate conversion.")
#define STR_SETTINGS_RESAMPLING_LABEL     NSLS(@"settings.playback.resampling_label",            @"Resampling",               @"iOS Settings, Playback screen: row choosing the quality used when a music file's sample rate differs from the output device's and the audio is converted. Use the standard audio-engineering term for sample-rate conversion.")
#define STR_SETTINGS_RESAMPLING_HIGH      NSLS(@"settings.playback.resampling.high",             @"High",                     @"iOS Settings, Playback screen, Resampling choice: high conversion quality, the default. An adjective describing quality.")
#define STR_SETTINGS_RESAMPLING_MAXIMUM   NSLS(@"settings.playback.resampling.maximum",          @"Maximum",                  @"iOS Settings, Playback screen, Resampling choice: the highest conversion quality, which uses more battery. An adjective describing quality.")
#define STR_SETTINGS_RESAMPLING_CAPTION   NSLS(@"settings.playback.resampling.caption",          @"Used when a file's sample rate differs from the output's. High differs from Maximum only above 21 kHz and uses about half the power.", @"iOS Settings, Playback screen: footer under the Resampling row. kHz is the international unit symbol for kilohertz and is not translated; keep the number 21.")
...
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_CAPTION      NSLS(@"settings.general.exclusive_output.caption", @"Stops other apps from using the output device while playing", @"Settings: exclusive output explanation. No trailing period.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_NEEDS_BIT_PERFECT NSLS(@"settings.general.exclusive_output.needs_bit_perfect", @"Enable bit-perfect output", @"Settings: why the exclusive output switch is disabled. No trailing period.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_UNSUPPORTED  NSLS(@"settings.general.exclusive_output.unsupported", @"This device does not support exclusive output", @"Settings: the selected device does not expose writable exclusive access. No trailing period.")
#define STR_SETTINGS_DECLICK                       NSLS(@"settings.general.declick", @"Declick", @"Settings, Audio pane: switch above Bit-perfect output. On, playback ramps the volume over the first and last few milliseconds of every start, stop, pause, seek and track change so the edge cannot click; off, it cuts, leaving every sample untouched. A short noun phrase for removing clicks.")
#define STR_SETTINGS_DECLICK_CAPTION               NSLS(@"settings.general.declick.caption", @"Uses volume ramps to reduce clicking", @"Settings: declick explanation. No trailing period.")
#define STR_SETTINGS_BIT_PERFECT_NEEDS_DEVICE  NSLS(@"settings.general.bit_perfect.needs_device",   @"Choose a wired device", @"Settings, Audio pane: caption under the disabled bit-perfect output switch while the chosen device is Bluetooth, AirPlay or another transport the mode cannot drive. The System Output row has its own caption below. No trailing period.")
#define STR_SETTINGS_BIT_PERFECT_SYSTEM_OUTPUT NSLS(@"settings.general.bit_perfect.system_output",  @"System Output cannot be used", @"Settings, Audio pane: caption under the disabled bit-perfect output switch while the Output list's System Output row is selected. That row follows whatever device macOS is using rather than naming one, so the mode has no device to drive — even when the system's device is itself wired. Name the row exactly as 'menu.output.system' translates it, so it matches what the list shows. No trailing period.")
#define STR_SETTINGS_BIT_PERFECT_IDLE          NSLS(@"settings.general.bit_perfect.idle",           @"On: nothing playing", @"Settings, Audio pane: caption under the bit-perfect output switch while it is on and playback is stopped or paused.")
#define STR_SETTINGS_BIT_PERFECT_ACTIVE        NSLS(@"settings.general.bit_perfect.active",         @"Active: %@",               @"Settings, Audio pane: caption under the bit-perfect output switch while the current track is delivered bit-perfect. %@ is the format, e.g. '96.0 kHz · 24-bit · exclusive'.")
#define STR_SETTINGS_BIT_PERFECT_FORMAT        NSLS(@"settings.general.bit_perfect.format",         @"%1$@ · %2$@-bit · shared",          @"The output format bit-perfect output settled on: %1$@ is the sample rate, already formatted with its unit (e.g. '96.0 kHz'), %2$@ the word length in bits.")
#define STR_SETTINGS_BIT_PERFECT_FORMAT_EXCLUSIVE NSLS(@"settings.general.bit_perfect.format_exclusive", @"%1$@ · %2$@-bit · exclusive", @"The output format bit-perfect output settled on, with the app holding exclusive use of the device: %1$@ is the sample rate, already formatted with its unit (e.g. '96.0 kHz'), %2$@ the word length in bits.")
#define STR_SETTINGS_BIT_PERFECT_RATE_UNSUPPORTED NSLS(@"settings.general.bit_perfect.rate_unsupported", @"Device does not support file's sample rate: playing at %@", @"Bit-perfect output status: the device cannot run at the file's sample rate, so the audio is resampled. %@ is the rate the device is running at, already formatted with its unit (e.g. '88.2 kHz').")
#define STR_SETTINGS_BIT_PERFECT_SWITCH_FAILED NSLS(@"settings.general.bit_perfect.switch_failed",  @"Failed to get device's audio settings", @"Bit-perfect output status: the output route, format or volume/balance/mute settings could not be confirmed.")
#define STR_SETTINGS_BIT_PERFECT_DEPTH         NSLS(@"settings.general.bit_perfect.depth",          @"File's bit depth is greater than device can play", @"Bit-perfect output status: the file carries more bits per sample than the decoder or the device can pass on, so the audio is truncated. The decoder is 32-bit float, which carries 24 integer bits.")
#define STR_SETTINGS_BIT_PERFECT_CHANNELS      NSLS(@"settings.general.bit_perfect.channels",       @"The playback path changes this file's channel count", @"Bit-perfect output status: the decoded file, mixer or output device has a different channel count, so unchanged delivery of every channel is not verified.")
#define STR_SETTINGS_BIT_PERFECT_MUTED         NSLS(@"settings.general.bit_perfect.muted",          @"The output device is muted", @"Bit-perfect output status: the device's separate mute control is on, even if its volume still reads 100 percent.")
#define STR_SETTINGS_BIT_PERFECT_VOLUME        NSLS(@"settings.general.bit_perfect.volume",         @"Device's volume is %@%%: set to 100%% for bit-perfect output", @"Bit-perfect output status: macOS is scaling the audio in software because the device's volume is below full. %@ is the volume as a whole-number percentage; %% is a literal percent sign.")
#define STR_SETTINGS_BIT_PERFECT_BALANCE       NSLS(@"settings.general.bit_perfect.balance", @"System balance not centered: center it for bit-perfect output", @"Bit-perfect output is scaling one channel because the system balance is not centered")
#define STR_SETTINGS_BIT_PERFECT_EXCLUSIVE_REFUSED NSLS(@"settings.general.bit_perfect.exclusive_refused", @"Could not obtain exclusive use of the device", @"Bit-perfect output status: exclusive access could not be confirmed, for example because another app holds the device or a device property read failed.")
#define STR_SETTINGS_BIT_PERFECT_LOSSY         NSLS(@"settings.general.bit_perfect.lossy",          @"File is lossy: decoded audio plays at its own sample rate, with no volume change", @"Bit-perfect output status: every other check holds, but the file is a lossy format (MP3, AAC). Its decoded audio reaches the device at the file's own sample rate with no volume change, rounded only to the device's bit depth, so it is not a lossless original.")
#define STR_SETTINGS_ANALYSIS_SECTION   NSLS(@"settings.playback.analysis_section",             @"Track analysis",                 @"Settings, Playback pane: heading above the group of rows enabling automatic tempo and key analysis.")
...
#define STR_SETTINGS_KEY_NOTATION_LABEL NSLS(@"settings.appearance.key_notation_label",         @"Key notation",           @"Settings, Appearance pane: label beside the choice of how the musical key is written.")
...
#define STR_SETTINGS_APPEARANCE_LABEL   NSLS(@"settings.appearance.appearance_label",           @"Window appearance",              @"Settings, Appearance pane: label beside the light/dark appearance choice.")
#define STR_SETTINGS_SHOW_TRAFFIC_LIGHTS NSLS(@"settings.appearance.show_traffic_lights",       @"Show window controls",      @"Settings, Appearance pane, Window group: switch showing or hiding the main window's custom close and minimize buttons. The buttons are the red and yellow macOS-style dots. The default is on.")
...
#define STR_SETTINGS_WINDOW_TINT_CUSTOM_DARK_LABEL NSLS(@"settings.appearance.window_tint_custom_dark_label", @"Dark color", @"Settings, Appearance pane: label beside the custom window tint color well used while the app is in dark mode.")
#define STR_SETTINGS_WINDOW_TINT_CUSTOM_LIGHT_LABEL NSLS(@"settings.appearance.window_tint_custom_light_label", @"Light color", @"Settings, Appearance pane: label beside the custom window tint color well used while the app is in light mode.")
#define STR_SETTINGS_THEME_COLOR_LABEL  NSLS(@"settings.appearance.theme.color_label",          @"Color",                   @"Settings, theme editor: label beside a single color well, replacing the Dark/Light pair's labels when the theme keeps one set of colors for both appearances.")
#define STR_SETTINGS_THEME_COLORS_LABEL NSLS(@"settings.appearance.theme.colors_label",         @"Colors",                  @"Settings, theme editor: label beside a pair of color wells (played and unplayed), replacing the Dark/Light labels when the theme keeps one set of colors for both appearances.")
#define STR_SETTINGS_WAVEFORM_LEVELS    NSLS(@"settings.appearance.waveform_section",           @"Waveform levels",          @"Appearance pane: disclosure row revealing waveform normalization and display gain, which apply to all themes.")
#define STR_SETTINGS_DISPLAY_PREFERENCES NSLS(@"settings.appearance.display_preferences",        @"Display preferences",      @"Appearance pane: group of app-wide display preferences below the current theme and theme library.")
#define STR_SETTINGS_WAVEFORM_NORMALIZE NSLS(@"settings.appearance.waveform_normalize",         @"Normalize waveform",       @"Settings, Appearance pane, Waveform group: switch that scales each track's waveform so its loudest passage draws at full height, whatever the file's mastering level. The audio term, as in normalizing a recording's level.")
#define STR_SETTINGS_WAVEFORM_GAIN      NSLS(@"settings.appearance.waveform_gain",              @"Waveform gain",            @"Settings, Appearance pane, Waveform group: row beside the slider scaling how tall the waveform draws — up grows it into the ceiling like a louder master, down shrinks it and shows more of a loud track's dynamic range. The word DJ mixers print on the input gain/trim knob.")
#define STR_SETTINGS_WAVEFORM_GAIN_VALUE NSLS(@"settings.appearance.waveform_gain_value",       @"%@ dB",                    @"Settings, Appearance pane: the waveform gain slider's live readout, e.g. '+3 dB' or '−6 dB'. %@ is the already-formatted signed number. Keep the 'dB' unit, in the locale's own spelling if it has one.")
#define STR_SETTINGS_THEME_WAVEFORM_STYLE NSLS(@"settings.appearance.theme.waveform_style",      @"Waveform style",           @"Settings, theme editor, Player group: row beside the dropdown choosing the waveform drawing style — the same choice as the Appearance pane's Style row, named in full here because the row sits among the player's other rows.")
#define STR_SETTINGS_THEME_WAVEFORM_BAR_DENSITY NSLS(@"settings.appearance.theme.waveform_bar_density", @"Waveform bar density", @"Theme editor: slider controlling the number of waveform bars or Wiggle loops. 100% is the style's original density; higher values draw more bars. Disabled for Detailed styles and the flat Cupertino Basic pill.")
#define STR_SETTINGS_THEME_WAVEFORM_BAR_WIDTH NSLS(@"settings.appearance.theme.waveform_bar_width", @"Waveform bar width", @"Theme editor: slider controlling waveform bar thickness, Wiggle stroke thickness, or Cupertino Basic pill height. 100% is the style's original thickness. Disabled for Detailed styles.")
#define STR_SETTINGS_THEME_WAVEFORM_COLOR NSLS(@"settings.appearance.theme.waveform_color",      @"Waveform color",           @"Settings, theme editor, Player group: row beside the dropdown choosing the waveform's color palette (as opposed to its drawing style).")
#define STR_SETTINGS_THEME_WAVEFORM_GRADIENT NSLS(@"settings.appearance.theme.waveform_gradient", @"Waveform gradient",        @"Settings, theme editor, Player group: switch enabling the vertical gradient shading on the waveform bars; off draws them in flat color.")
...
#define STR_SETTINGS_WAVEFORM_CUSTOM_DARK_LABEL NSLS(@"settings.appearance.waveform_custom_dark_label", @"Dark colors",     @"Settings, Appearance pane: label beside the custom waveform color wells used while the app is in dark mode.")
#define STR_SETTINGS_WAVEFORM_CUSTOM_LIGHT_LABEL NSLS(@"settings.appearance.waveform_custom_light_label", @"Light colors",  @"Settings, Appearance pane: label beside the custom waveform color wells used while the app is in light mode.")
...
#define STR_SETTINGS_WAVEFORM_DRAG_LABEL NSLS(@"settings.appearance.waveform_drag_label",       @"Dragging the waveform",           @"Settings, General pane, Window group: row beside the dropdown choosing what dragging the mouse on the waveform does. The key keeps its historical settings.appearance prefix.")
...
#define STR_SETTINGS_WAVEFORM_DRAG_SEEK NSLS(@"settings.appearance.waveform_drag.seek",         @"Scrub through track",                     @"Settings, Appearance pane, waveform drag choice: dragging on the waveform scrubs through the track like a slider — playback jumps to where the mouse is released — and the window stays put.")
#define STR_SETTINGS_ARTWORK_DRAG_LABEL NSLS(@"settings.general.artwork_drag_label",           @"Artwork drag",            @"Settings, General pane, Window group: row beside the dropdown choosing what dragging the album artwork out of the window hands to the app it is dropped on.")
...
#define STR_SETTINGS_THEME_GROUP_USER          NSLS(@"settings.appearance.theme.group_user",          @"Custom",                         @"Settings, Appearance pane: header row in the theme list, above the themes the user created, duplicated or imported. Shown only once at least one exists.")
...
#define STR_SETTINGS_THEME_EDIT                NSLS(@"settings.appearance.theme.edit",                @"Edit Theme…",                  @"Appearance pane: primary button opening the current custom theme's editor. Ends with a real ellipsis character (…).")
#define STR_SETTINGS_THEME_CUSTOMIZE           NSLS(@"settings.appearance.theme.customize",           @"Customize…",                   @"Appearance pane: primary button for a built-in theme; copies its current appearance into an editable custom theme and opens the editor.")
...
#define STR_SETTINGS_THEME_RANDOMIZE           NSLS(@"settings.appearance.theme.randomize",           @"Randomize",                    @"Settings toolbar: label of the theme editor's dice control — roll settings, roll colors, undo — (icon-only on screen; read by VoiceOver and the customize sheet).")
#define STR_SETTINGS_THEME_RANDOMIZE_SETTINGS  NSLS(@"settings.appearance.theme.randomize_settings",  @"Randomize settings",           @"Settings toolbar: tooltip and accessibility name of the first dice — rolls the theme's appearance choices (backgrounds, waveform, buttons, fonts) but never its colors.")
#define STR_SETTINGS_THEME_RANDOMIZE_COLORS    NSLS(@"settings.appearance.theme.randomize_colors",    @"Randomize colors",             @"Settings toolbar: tooltip and accessibility name of the second dice — rolls a new color palette for the theme and nothing else.")
...
#define STR_SETTINGS_THEME_APPEARANCE          NSLS(@"settings.appearance.theme.appearance",          @"Theme color modes",                   @"Theme editor, Window group: row beside the dropdown choosing whether the theme keeps one color set used in every appearance (single mode) or separate colors for light and dark.")
...
#define STR_SETTINGS_ICON_SECTION              NSLS(@"settings.appearance.icon_section",              @"App icon",                         @"Theme editor: heading above the app icon and Dock icon rows, the first card of the page.")
#define STR_SETTINGS_THEME_SHOW_TRANSPORT_BUTTONS NSLS(@"settings.appearance.theme.show_transport_buttons", @"Show playback buttons", @"Theme editor, Transport group: show or hide the playlist, play/pause and next buttons. Their appearance controls disable while hidden.")
#define STR_SETTINGS_THEME_SHOW_STATUS_ICONS      NSLS(@"settings.appearance.theme.show_status_icons",      @"Show status icons",      @"Theme editor, Info group: show or hide the active FX symbols and bit-perfect lock indicator. This changes visibility only, not the audio processing.")
#define STR_SETTINGS_THEME_SHOW_TIME_LABELS       NSLS(@"settings.appearance.theme.show_time_labels",       @"Show time labels",       @"Theme editor, Time display group: show or hide both the elapsed and total/remaining time readouts.")
#define STR_SETTINGS_TRANSPORT_SECTION         NSLS(@"settings.appearance.transport_section",         @"Playback buttons",                    @"Theme editor: heading above the transport rows — the playlist, play and next buttons' symbols, colors and images, and the gradient behind them.")
#define STR_SETTINGS_PLAYER_SECTION            NSLS(@"settings.appearance.player_section",            @"Player display",                       @"Theme editor: heading above the player rows — default artwork, the title and artist fonts and colors.")
#define STR_SETTINGS_INFO_SECTION              NSLS(@"settings.appearance.info_section",              @"Track information",                         @"Theme editor: heading above the shared readout font, status icons, file info, tempo and key settings.")
...
#define STR_SETTINGS_THEME_PLAYLIST_BACKGROUND NSLS(@"settings.appearance.theme.playlist_background", @"Background",                   @"Theme editor, Playlist group: row beside the dropdown choosing the playlist background — the frosted glass look, or a solid color.")
...
#define STR_SETTINGS_THEME_PLAYLIST_TINT       NSLS(@"settings.appearance.theme.playlist_tint_label", @"Background tint",              @"Theme editor, Playlist group: row beside the dropdown choosing the translucent color wash laid over the playlist background — the same choices as the window's background tint.")
#define STR_SETTINGS_THEME_PLAYING_ROW         NSLS(@"settings.appearance.theme.playing_row",         @"Playing row background",                  @"Theme editor, Playlist group: row beside the color wells filling the playing track's row.")
#define STR_SETTINGS_THEME_SELECTED_ROW        NSLS(@"settings.appearance.theme.selected_row",        @"Selected row background",                 @"Theme editor, Playlist group: row beside the color wells filling selected rows.")
#define STR_SETTINGS_THEME_FONT_MAIN           NSLS(@"settings.appearance.theme.font_main",           @"Title font",                    @"Theme editor, Player group: row for the header title font.")
#define STR_SETTINGS_THEME_FONT_ARTIST         NSLS(@"settings.appearance.theme.font_artist",         @"Artist font",                   @"Theme editor, Player group: row for the header artist-line font.")
...
#define STR_SETTINGS_THEME_FONT_PLAYLIST_DURATION NSLS(@"settings.appearance.theme.font_playlist_duration", @"Playlist duration font",     @"Theme editor, Fonts group: row for the font of the playlist's duration column.")
#define STR_SETTINGS_THEME_PLAYLIST_NUMBER_COLUMN NSLS(@"settings.appearance.theme.playlist_number_column", @"Show number column",        @"Theme editor, Playlist group: switch showing or hiding the playlist's leading row-number column, which also carries the playing indicator.")
#define STR_SETTINGS_THEME_PLAYLIST_ARTWORK    NSLS(@"settings.appearance.theme.playlist_artwork",    @"Show artwork column",               @"Theme editor, Playlist group: switch showing or hiding the playlist's album-art column.")
#define STR_SETTINGS_THEME_ALBUM_ART           NSLS(@"settings.appearance.theme.album_art",           @"Default artwork",                    @"Theme editor, Transport group: row beside the artwork preview pair (one per appearance under Light & Dark Modes, one under Single Mode) showing the placeholder image used when a track has no artwork; clicking a preview opens a file panel to pick a square JPEG or PNG.")
#define STR_SETTINGS_THEME_IMAGE_CLEAR         NSLS(@"settings.appearance.theme.image_clear",         @"Clear custom image",                 @"Theme editor, accessibility label of the small ✕ badge revealed when hovering an image preview — the default artwork, the app icon or a transport button's image: removes that preview's picked custom image and returns to the factory one.")
#define STR_SETTINGS_THEME_IMAGE_MISSING       NSLS(@"settings.appearance.theme.image_missing",       @"Image missing",                      @"Theme editor, tooltip on the red (!) badge at an image preview's leading corner: the image this theme picked is no longer on this Mac, so the preview is drawing the factory image instead. Clearing the image removes the badge.")
#define STR_SETTINGS_THEME_ALBUM_ART_INVALID   NSLS(@"settings.appearance.theme.album_art_invalid",   @"This image can’t be used.", @"Alert title when a picked image — default artwork, app icon or a button image — fails validation.")
#define STR_SETTINGS_THEME_ALBUM_ART_REQUIREMENTS NSLS(@"settings.appearance.theme.album_art_requirements", @"The image must be a square JPEG or PNG, between 64 and 4096 pixels, under 8 MB.", @"Alert detail under the invalid-image title, stating every requirement.")
#define STR_SETTINGS_THEME_APP_ICON            NSLS(@"settings.appearance.theme.app_icon",            @"App icon",                           @"Theme editor, Window group: row beside the app icon preview (click to pick a square JPEG or PNG that replaces the app's icon in the Dock and app switcher), and the Dock icon dropdown's choice that keeps that icon showing while a track plays.")
#define STR_SETTINGS_THEME_DOCK_ICON           NSLS(@"settings.appearance.theme.dock_icon",           @"Dock icon",                          @"Theme editor, Window group: row beside the dropdown choosing what the Dock shows while a track with artwork plays — the album art, or the app icon.")
#define STR_SETTINGS_THEME_APP_ICON_SHAPE      NSLS(@"settings.appearance.theme.app_icon_shape",      @"App icon shape",               @"Theme editor, Icon group: switch that shapes the Dock's album art and a custom app icon the way macOS shapes app icons (rounded, inset on the icon grid); off shows the picture exactly as it is. The default is on.")
#define STR_SETTINGS_THEME_DOCK_ICON_ALBUM_ART NSLS(@"settings.appearance.theme.dock_icon.album_art", @"Album art",                          @"Dock icon choice: the playing track's cover art replaces the app icon in the Dock. The default.")
#define STR_SETTINGS_THEME_CUSTOM_CORNER_RADIUS NSLS(@"settings.appearance.theme.custom_corner_radius", @"Custom corner radius",             @"Theme editor, Window group: switch above the corner-radius slider. Off, the window's corners use the standard macOS radius; on, the slider's value.")
#define STR_SETTINGS_THEME_BUTTON_PLAYLIST     NSLS(@"settings.appearance.theme.button_playlist",     @"Playlist button",                    @"Theme editor, Player group: row beside the dropdown choosing the playlist toggle button's symbol, or a custom image.")
#define STR_SETTINGS_THEME_BUTTON_PLAY         NSLS(@"settings.appearance.theme.button_play",         @"Play button",                        @"Theme editor, Player group: row beside the dropdown choosing the play/pause button's symbol pair, or custom images.")
#define STR_SETTINGS_THEME_BUTTON_NEXT         NSLS(@"settings.appearance.theme.button_next",         @"Next button",                        @"Theme editor, Player group: row beside the dropdown choosing the next-track button's symbol, or a custom image.")
#define STR_SETTINGS_THEME_BUTTON_PLAYLIST_COLOR NSLS(@"settings.appearance.theme.button_playlist_color", @"Playlist button color",          @"Theme editor, Transport group: row beside the playlist button's symbol color wells — the Dark one drawn over dark artwork, the Light one over light artwork, decided by the art under the buttons rather than the appearance.")
#define STR_SETTINGS_THEME_BUTTON_PLAY_COLOR   NSLS(@"settings.appearance.theme.button_play_color",   @"Play button color",                  @"Theme editor, Transport group: row beside the play/pause button's symbol color wells — the Dark one drawn over dark artwork, the Light one over light artwork.")
#define STR_SETTINGS_THEME_BUTTON_NEXT_COLOR   NSLS(@"settings.appearance.theme.button_next_color",   @"Next button color",                  @"Theme editor, Transport group: row beside the next-track button's symbol color wells — the Dark one drawn over dark artwork, the Light one over light artwork.")
#define STR_SETTINGS_THEME_BUTTON_PLAYLIST_IMAGE NSLS(@"settings.appearance.theme.button_playlist_image", @"Playlist button images",         @"Theme editor, Transport group: row beside the playlist button's custom image previews, one drawn over dark artwork and one over light; shown once a custom image replaces the symbol.")
#define STR_SETTINGS_THEME_BUTTON_PLAY_IMAGE   NSLS(@"settings.appearance.theme.button_play_image",   @"Play button images",                 @"Theme editor, Transport group: row beside the play state's two custom image previews, one drawn over dark artwork and one over light; shown once a custom image replaces the symbols.")
#define STR_SETTINGS_THEME_BUTTON_PAUSE_IMAGE  NSLS(@"settings.appearance.theme.button_pause_image",  @"Pause button images",                @"Theme editor, Transport group: row beside the pause state's two custom image previews, one drawn over dark artwork and one over light; shown with the play images once a custom image replaces the symbols.")
#define STR_SETTINGS_THEME_BUTTON_NEXT_IMAGE   NSLS(@"settings.appearance.theme.button_next_image",   @"Next button images",                 @"Theme editor, Transport group: row beside the next-track button's custom image previews, one drawn over dark artwork and one over light; shown once a custom image replaces the symbol.")
#define STR_SETTINGS_THEME_BUTTON_CUSTOM_IMAGE NSLS(@"settings.appearance.theme.button_custom_image", @"Custom image…",                      @"Theme editor, last item of each transport button's symbol dropdown: opens a file panel to pick a square JPEG or PNG drawn in place of the symbol. Ends with a real ellipsis character (…), not three periods.")
#define STR_SETTINGS_THEME_BUTTON_GRADIENT     NSLS(@"settings.appearance.theme.button_gradient",     @"Button gradient",                    @"Theme editor, Playback buttons group: chooses when the dark gradient appears behind the buttons.")
#define STR_SETTINGS_THEME_BUTTON_GRADIENT_NONE NSLS(@"settings.appearance.theme.button_gradient.none", @"None", @"Button gradient choice: never darken the artwork behind the playback buttons.")
#define STR_SETTINGS_THEME_BUTTON_GRADIENT_HOVER NSLS(@"settings.appearance.theme.button_gradient.hover", @"Show on hover", @"Button gradient choice: show the gradient while the pointer is over the player window.")
#define STR_SETTINGS_THEME_BUTTON_GRADIENT_ARTWORK NSLS(@"settings.appearance.theme.button_gradient.artwork", @"Show on artwork", @"Button gradient choice: show the gradient over a track's cover artwork, but hide it over the theme's default image.")
#define STR_SETTINGS_THEME_BUTTON_GRADIENT_ALWAYS NSLS(@"settings.appearance.theme.button_gradient.always", @"Always", @"Button gradient choice: always show the gradient behind the playback buttons.")
#define STR_SETTINGS_THEME_PLAYLIST_DURATION_COLUMN NSLS(@"settings.appearance.theme.playlist_duration_column", @"Show duration column",    @"Theme editor, Playlist group: switch showing or hiding the playlist's track-duration column.")
#define STR_SETTINGS_THEME_EDITOR_TITLE        NSLS(@"settings.appearance.theme.editor_title",        @"Theme: %@",                    @"Settings window title while the theme editor page is showing. %@ is the theme's name, e.g. 'Theme: Cupertino'.")
#define STR_SETTINGS_THEME_PLAYLIST_NUMBER_COLOR_CUSTOM NSLS(@"settings.appearance.theme.playlist_number_color_custom", @"Custom row number color", @"Theme editor, Playlist group: switch giving the playlist's row-number column its own text color instead of the artist color it normally shares.")
#define STR_SETTINGS_THEME_PLAYLIST_NUMBER_COLOR NSLS(@"settings.appearance.theme.playlist_number_color", @"Row number color",       @"Theme editor, Playlist group: row beside the color wells for the playlist's row-number column, shown while its custom color switch is on.")
#define STR_SETTINGS_THEME_PLAYLIST_TITLE_COLOR_CUSTOM NSLS(@"settings.appearance.theme.playlist_title_color_custom", @"Custom title color",  @"Theme editor, Playlist group: switch giving the playlist's track titles their own text color instead of the header's title color they normally share.")
#define STR_SETTINGS_THEME_PLAYLIST_TITLE_COLOR NSLS(@"settings.appearance.theme.playlist_title_color",   @"Title color",            @"Theme editor, Playlist group: row beside the color wells for the playlist's track titles, shown while their custom color switch is on.")
#define STR_SETTINGS_THEME_PLAYLIST_ARTIST_COLOR_CUSTOM NSLS(@"settings.appearance.theme.playlist_artist_color_custom", @"Custom artist color", @"Theme editor, Playlist group: switch giving the playlist's artist lines their own text color instead of the header's artist color they normally share.")
#define STR_SETTINGS_THEME_PLAYLIST_ARTIST_COLOR NSLS(@"settings.appearance.theme.playlist_artist_color", @"Artist color",           @"Theme editor, Playlist group: row beside the color wells for the playlist's artist lines, shown while their custom color switch is on.")
#define STR_SETTINGS_THEME_PLAYLIST_DURATION_COLOR_CUSTOM NSLS(@"settings.appearance.theme.playlist_duration_color_custom", @"Custom duration color", @"Theme editor, Playlist group: switch giving the playlist's track-duration column its own text color instead of the artist color it normally shares.")
#define STR_SETTINGS_THEME_PLAYLIST_DURATION_COLOR NSLS(@"settings.appearance.theme.playlist_duration_color", @"Duration color",       @"Theme editor, Playlist group: row beside the color wells for the playlist's track-duration column, shown while its custom color switch is on.")
...
#define STR_SETTINGS_THEME_IMPORT_FAILED_SOME  NSLS(@"settings.appearance.theme.import_failed_some", @"Some files aren't Vibe themes.", @"Alert shown when several files were imported at once and more than one of them could not be read as a theme. The files that failed are listed below it. Deliberately carries no count, so it needs no plural agreement.")
...
#define STR_SETTINGS_CONVERT_ENABLED     NSLS(@"settings.convert.enabled",                      @"Enable FLAC conversion",                  @"Settings, Files pane, FLAC conversion group: switch showing or hiding the whole Convert feature — off removes the Convert menu from the menu bar and the Convert to FLAC item from the context menus.")
#define STR_SETTINGS_CONVERT_DEST_LABEL  NSLS(@"settings.convert.destination_label",            @"Save converted files:",    @"Settings, Files pane, FLAC conversion group: label beside the dropdown choosing where Convert to FLAC writes its output. Ends with a colon.")
...
#define STR_SETTINGS_FOLDER_SORT_NEWEST_FIRST NSLS(@"settings.files.folder_sort.newest_first", @"Date modified, newest first", @"Settings, Files pane, folder open sort choice: order the songs by the date each file was last changed, most recent first — the same date the Finder and the iPhone Files app call Date Modified.")
#define STR_SETTINGS_FOLDER_SORT_AS_RECEIVED  NSLS(@"settings.files.folder_sort.as_received",  @"Unsorted",                 @"Settings, Files pane, folder open sort choice: do not reorder the songs at all — keep whatever order the disk or the cloud storage service listed them in.")
...
#define STR_SETTINGS_PERMISSIONS_LABEL    NSLS(@"settings.permissions.label",                   @"Folder access:",             @"Settings, Files pane: label beside the list of folders the app may read, and the paragraph above it. Ends with a colon.")
#define STR_SETTINGS_PERMISSIONS_EXPLAIN  NSLS(@"settings.permissions.explain",                 @"Add folders to avoid repeated permission prompts. Removing access does not delete files.", @"Settings, Files pane: explanatory text above the list of granted folders.")
...
#define STR_SETTINGS_REMOVE_FOLDER        NSLS(@"settings.permissions.remove",                  @"Remove Access",                   @"Settings, Files pane: button that removes the selected folder from the granted list, giving up the app's access to it.")

#define STR_SETTINGS_ALLOW_BIT_PERFECT_ANY_DEVICE NSLS(@"settings.advanced.allow_bit_perfect_any_device", @"Allow bit-perfect on any device", @"Settings, Advanced pane: testing switch that permits enabling the bit-perfect output mode on every output transport, including Bluetooth. Use the same term for bit-perfect as the Audio Output pane.")
...
#define STR_SETTINGS_FACTORY_RESET_LABEL  NSLS(@"settings.advanced.factory_reset_label",         @"Factory reset",           @"Settings, Advanced pane: row label and button that reset all settings and delete custom themes.")
#define STR_SETTINGS_RESET_DEFAULTS       NSLS(@"settings.advanced.reset_defaults",             @"Reset to defaults",        @"Settings, Advanced pane: button that resets all settings while preserving custom themes. Disabled while settings are already at defaults.")
#define STR_SETTINGS_RESET_LABEL          NSLS(@"settings.advanced.reset_label",                @"Reset",                    @"Settings, Advanced pane: row label beside Reset to defaults.")
#define STR_SETTINGS_RESET_CAPTION        NSLS(@"settings.advanced.reset_caption",              @"Reset all settings to defaults, keeps custom themes", @"Settings, Advanced pane: hint below Reset; custom themes and their images are kept.")
#define STR_SETTINGS_FACTORY_RESET_CAPTION NSLS(@"settings.advanced.factory_reset_caption",     @"Reset to factory defaults, including deleting custom themes", @"Settings, Advanced pane: hint below Factory reset; custom themes are deleted as well as resetting settings.")
...
#define STR_SETTINGS_AUDIO_PLAYED_LABEL   NSLS(@"settings.advanced.audio_played_label",         @"Listening time",           @"Settings, About pane on both platforms, statistics: label beside the total listening time, shown in words like '3 days, 4 hours'.")

#define STR_SETTINGS_AUDIO_PATH_SECTION        NSLS(@"settings.advanced.audio_path.section",        @"Audio",                     @"Settings, Advanced pane: heading above the rows listing the audio path — every stage the sound passes through, from the source file to the output device.")
#define STR_SETTINGS_AUDIO_PATH_SOURCE         NSLS(@"settings.advanced.audio_path.source_label",   @"Source file:",              @"Settings, Advanced pane, Audio group: label beside the current file's format — codec, sample rate, bit depth, channels. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_DECODE         NSLS(@"settings.advanced.audio_path.decode_label",   @"Decoder:",                  @"Settings, Advanced pane, Audio group: label beside how the file is decoded and converted for the mixing bus. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_BUS            NSLS(@"settings.advanced.audio_path.bus_label",      @"Mixing bus:",               @"Settings, Advanced pane, Audio group: label beside the internal mixing bus's format — the sample rate and depth everything is mixed at. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_PITCH          NSLS(@"settings.advanced.audio_path.pitch_label",    @"Pitch:",                    @"Settings, Advanced pane, Audio group: label beside the pitch fader's resampler — off at zero pitch, else the pitch and the delay it adds. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_FX             NSLS(@"settings.advanced.audio_path.fx_label",       @"Effects:",                  @"Settings, Advanced pane, Audio group: label beside the DJ effects stage — off, or which effects are active. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_METER          NSLS(@"settings.advanced.audio_path.meter_label",    @"Level meter:",              @"Settings, Advanced pane, Audio group: label beside the analyzer behind the equalizer bars — on with its rate while the bars are shown, else off. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_OUTPUT         NSLS(@"settings.advanced.audio_path.output_label",   @"Output:",                   @"Settings, Advanced pane, Audio group: label beside the format handed to the audio system — rate, depth, channels — and whether it is running. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_DEVICE         NSLS(@"settings.advanced.audio_path.device_label",   @"Output device:",            @"Settings, Advanced pane, Audio group: label beside the hardware device the output reaches and the format it runs at. Ends with a colon.")
#define STR_SETTINGS_AUDIO_PATH_NONE           NSLS(@"settings.advanced.audio_path.none",           @"None",                      @"Settings, Advanced pane, Audio group: readout of a stage that has nothing — no file loaded, no output device.")
#define STR_SETTINGS_AUDIO_PATH_DIRECT         NSLS(@"settings.advanced.audio_path.direct",         @"Read as is",                @"Settings, Advanced pane, Audio group: decoder readout when the file needs no conversion — its samples reach the mixing bus unchanged.")
#define STR_SETTINGS_AUDIO_PATH_RESAMPLED      NSLS(@"settings.advanced.audio_path.resampled",      @"Resampled %1$@ → %2$@ at maximum quality", @"Settings, Advanced pane, Audio group: decoder readout when the file's sample rate is converted, e.g. 'Resampled 44.1 kHz → 48 kHz at maximum quality'. %1$@ and %2$@ are already-formatted sample rates; keep the arrow between them.")
#define STR_SETTINGS_AUDIO_PATH_MIXED          NSLS(@"settings.advanced.audio_path.mixed",          @"%1$@ mixed to %2$@",        @"Settings, Advanced pane, Audio group: decoder readout when the file's channels are folded, e.g. 'Mono mixed to Stereo' or '6 channels mixed to Stereo'. %1$@ is the file's channel description, %2$@ the bus's.")
#define STR_SETTINGS_AUDIO_PATH_BITS           NSLS(@"settings.advanced.audio_path.bits",           @"%@-bit",                    @"Settings, Advanced pane, Audio group: a sample depth, e.g. '24-bit'. %@ is the number of bits.")
#define STR_SETTINGS_AUDIO_PATH_FLOAT          NSLS(@"settings.advanced.audio_path.float",          @"32-bit float",              @"Settings, Advanced pane, Audio group: the floating-point sample format the bus mixes in. 'float' is the programming term for floating point and may stay in English.")
#define STR_SETTINGS_AUDIO_PATH_MONO           NSLS(@"settings.advanced.audio_path.mono",           @"Mono",                      @"Settings, Advanced pane, Audio group: a one-channel format.")
#define STR_SETTINGS_AUDIO_PATH_STEREO         NSLS(@"settings.advanced.audio_path.stereo",         @"Stereo",                    @"Settings, Advanced pane, Audio group: a two-channel format.")
#define STR_SETTINGS_AUDIO_PATH_CHANNELS       NSLS(@"settings.advanced.audio_path.channels",       @"%@ channels",               @"Settings, Advanced pane, Audio group: a format with more than two channels, e.g. '6 channels'. %@ is the already-formatted count.")
#define STR_SETTINGS_AUDIO_PATH_CHANNEL_COUNT  NSLS(@"settings.advanced.audio_path.channel_count",  @"%@ch",                      @"Settings, Advanced pane, Audio group: a stage's channel count in the compact form audio gear prints, e.g. '2ch'. %@ is the already-formatted count; keep it short, since every stage's row carries one.")
#define STR_SETTINGS_AUDIO_PATH_ON             NSLS(@"settings.advanced.audio_path.on",             @"On",                        @"Settings, Advanced pane, Audio group: readout of a stage that is in the path and working.")
#define STR_SETTINGS_AUDIO_PATH_OFF            NSLS(@"settings.advanced.audio_path.off",            @"Off",                       @"Settings, Advanced pane, Audio group: readout of a stage that is not in the path — the pitch at zero, the effects disabled, the meter not shown.")
#define STR_SETTINGS_AUDIO_PATH_IDLE           NSLS(@"settings.advanced.audio_path.idle",           @"Idle",                      @"Settings, Advanced pane, Audio group: the output while nothing plays — it is not running.")
#define STR_SETTINGS_AUDIO_PATH_RUNNING        NSLS(@"settings.advanced.audio_path.running",        @"Running",                   @"Settings, Advanced pane, Audio group: the output while it is delivering audio to the device.")
#define STR_SETTINGS_AUDIO_PATH_WAITING_TO_IDLE NSLS(@"settings.advanced.audio_path.waiting_to_idle", @"Waiting to idle",          @"Settings, Advanced pane, Audio group: the output is still running after playback stopped or paused, and stops on its own within seconds unless playback resumes.")
#define STR_SETTINGS_AUDIO_PATH_MILLISECONDS   NSLS(@"settings.advanced.audio_path.milliseconds",   @"%@ ms",                     @"Settings, Advanced pane, Audio group: a stage's latency, the last column of its row, e.g. '1.0 ms'. %@ is the already-formatted milliseconds; ms is the unit symbol and is not translated.")
#define STR_SETTINGS_AUDIO_PATH_VOICES         NSLS(@"settings.advanced.audio_path.voices",         @"Voices: %@",                @"Settings, Advanced pane, Audio group: how many files the bus is mixing at once — more than one while a track fades into the next. %@ is the already-formatted count.")
#define STR_SETTINGS_AUDIO_PATH_NOTHING_ACTIVE NSLS(@"settings.advanced.audio_path.nothing_active", @"On, nothing active",        @"Settings, Advanced pane, Audio group: the effects stage is enabled but no effect is applied right now.")
#define STR_SETTINGS_AUDIO_PATH_BIT_PERFECT    NSLS(@"settings.advanced.audio_path.bit_perfect",    @"BP",                        @"Settings, Advanced pane, Audio group: the output device row's tag while bit-perfect output is active — a debug readout, so the abbreviation of 'bit-perfect' is kept as is.")
#define STR_SETTINGS_AUDIO_PATH_HOG            NSLS(@"settings.advanced.audio_path.hog",            @"Hog",                       @"Settings, Advanced pane, Audio group: the output device row's tag while Vibe holds the device exclusively ('hog mode', CoreAudio's term) — a debug readout, kept as is.")
...
#define STR_SETTINGS_DEBUG_INFO_CAPTION   NSLS(@"settings.advanced.debug_info_caption",         @"Saves settings, audio devices, and this session's log for a bug report", @"Settings, Advanced pane, Build group: hint below Debug info saying what the saved file contains. No trailing period.")
...
#define STR_LABEL_FAVORITES_EMPTY_MESSAGE NSLS(@"label.favorites.empty.message", @"Add a folder from the Files tab, or star the one you have open, to keep it here.", @"iOS: explanatory line under the empty favorites headline. It names both ways to make a favorite, each in the words its own screen uses: the Files tab's long-press menu item says 'Add to Favorites', while the playlist screen's control is a star. 'Here' is the Favorites tab the line is shown on.")
...
#define STR_MENU_PLAYLIST_CLEAR          NSLS(@"menu.playlist.clear",          @"Clear Playlist",        @"iOS: destructive menu item on the playlist screen's bar button; empties the playlist and stops playback. It deletes no files — it only unloads what is queued — so the wording says Clear rather than Delete.")
...
#define STR_MENU_CONTEXT_ADD_FAVORITE    NSLS(@"menu.context.add_favorite",    @"Add to %@ Favorites",   @"iOS: long-press menu item on a FOLDER in the Files tab; stars it so it appears on the Favorites tab, without opening it or changing the playlist. %@ is the app name. It is named because the system's own 'Favorite' item sits in the same menu, and without it the two read as near-duplicates and neither says whose favorites it means. Keep the %@ — the app name is not translated, but the grammar around it is.")
...
#define STR_A11Y_PLAYLIST_MENU     NSLS(@"a11y.playlist.menu",     @"Options",               @"iOS: accessibility label for the gear button on the playlist screen's bar. It no longer opens Settings directly — it opens a menu holding Settings and Clear Playlist — so the label names the menu rather than either item.")
...
#define STR_SETTINGS_WAVEFORM_LEVELS_UNAVAILABLE NSLS(@"settings.appearance.waveform_levels_unavailable", @"%@ does not support waveform height", @"macOS Settings: caption when the selected waveform style does not support waveform height controls. %@ is the localized waveform style name.")
#define STR_SETTINGS_THEME_CURRENT NSLS(@"settings.appearance.current_theme", @"Current theme", @"macOS Settings: Current theme")
#define STR_SETTINGS_THEME_MODIFIED NSLS(@"settings.appearance.modified", @"%@ (Modified)", @"macOS Settings: %@ (Modified)")
#define STR_SETTINGS_THEME_REVERT NSLS(@"settings.appearance.revert", @"Revert", @"macOS Settings: Revert")
#define STR_SETTINGS_THEME_PREVIEW NSLS(@"settings.appearance.preview", @"Preview appearance", @"macOS Settings: Preview appearance")
#define STR_SETTINGS_THEME_SINGLE_CAPTION NSLS(@"settings.appearance.single_caption", @"This theme uses one palette and keeps the window in dark appearance", @"macOS Settings: caption explaining that a single-palette theme keeps the window in dark appearance.")
#define STR_SETTINGS_THEME_ON_DARK_ART NSLS(@"settings.appearance.on_dark_art", @"On dark artwork", @"macOS Settings: On dark artwork")
#define STR_SETTINGS_THEME_ON_LIGHT_ART NSLS(@"settings.appearance.on_light_art", @"On light artwork", @"macOS Settings: On light artwork")
#define STR_SETTINGS_THEME_UNDO_REMOVE NSLS(@"settings.appearance.undo_remove", @"Undo Remove Theme", @"macOS Settings: Undo Remove Theme")
#define STR_SETTINGS_THEME_UNDO_EDIT NSLS(@"settings.appearance.undo_edit", @"Undo Theme Edit", @"macOS Settings: Undo Theme Edit")
#define STR_SETTINGS_THEME_REDO_REMOVE NSLS(@"settings.appearance.redo_remove", @"Redo Remove Theme", @"macOS Edit menu and toolbar: repeat the theme removal that was undone.")
#define STR_SETTINGS_THEME_REDO_EDIT NSLS(@"settings.appearance.redo_edit", @"Redo Theme Edit", @"macOS Edit menu and toolbar: repeat a theme edit that was undone.")
