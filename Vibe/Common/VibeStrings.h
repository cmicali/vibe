...
// open-lock tooltip.
#define STR_SETTINGS_BIT_PERFECT               NSLS(@"settings.general.bit_perfect",                @"Bit-perfect output",       @"Settings, General pane, Audio group: switch that sends each file's audio to the chosen output device unchanged — the device's sample rate and bit depth follow the file, and the app keeps exclusive use of the device while playing.")
#define STR_SETTINGS_BIT_PERFECT_CAPTION_OFF   NSLS(@"settings.general.bit_perfect.caption_off",    @"Sets the device to each file's sample rate and bit depth, and keeps other apps off it while playing", @"Settings, General pane: caption under the bit-perfect output switch while it is off, explaining what turning it on does. No trailing period.")
...
#define STR_SETTINGS_BIT_PERFECT_FORMAT        NSLS(@"settings.general.bit_perfect.format",         @"%1$@ · %2$@-bit",          @"The output format bit-perfect output settled on: %1$@ is the sample rate, already formatted with its unit (e.g. '96.0 kHz'), %2$@ the word length in bits.")
