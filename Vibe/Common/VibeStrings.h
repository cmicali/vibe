...
#define STR_SETTINGS_OFF_WHILE_BIT_PERFECT NSLS(@"settings.playback.off_while_bit_perfect",      @"Off while bit-perfect output is on", @"Settings, Playback pane: caption under the disabled audio-FX switch and the disabled crossfade dropdown while bit-perfect output (Settings > General) is on, which turns both off.")
...
// open-lock tooltip.
#define STR_SETTINGS_BIT_PERFECT               NSLS(@"settings.general.bit_perfect",                @"Bit-perfect output",       @"Settings, General pane, Audio group: switch that sends each file's audio to the chosen output device unchanged — the device's sample rate and bit depth follow the file.")
#define STR_SETTINGS_BIT_PERFECT_CAPTION_OFF   NSLS(@"settings.general.bit_perfect.caption_off",    @"Sets the device to each file's sample rate and bit depth", @"Settings, General pane: caption under the bit-perfect output switch while it is off, explaining what turning it on does. No trailing period.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT              NSLS(@"settings.general.exclusive_output", @"Exclusive output", @"Settings, General pane: optional exclusive device access beneath Bit-perfect output.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_CAPTION      NSLS(@"settings.general.exclusive_output.caption", @"Keeps other apps from using the output device while playing", @"Settings: exclusive output explanation. No trailing period.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_NEEDS_BIT_PERFECT NSLS(@"settings.general.exclusive_output.needs_bit_perfect", @"Turn on bit-perfect output to enable exclusive output", @"Settings: why the exclusive output switch is disabled. No trailing period.")
#define STR_SETTINGS_EXCLUSIVE_OUTPUT_NEEDS_DEVICE NSLS(@"settings.general.exclusive_output.needs_device", @"Choose a physical device other than the system output to enable exclusive output", @"Settings: exclusive output is unavailable for virtual devices and the system output device. No trailing period.")
#define STR_SETTINGS_BIT_PERFECT_NEEDS_DEVICE  NSLS(@"settings.general.bit_perfect.needs_device",   @"Choose a wired output device in the Output menu to enable bit-perfect output", @"Settings, General pane: caption under the disabled bit-perfect output switch while the output is System Output or a Bluetooth, AirPlay or other device the mode cannot drive. No trailing period.")
#define STR_SETTINGS_BIT_PERFECT_IDLE          NSLS(@"settings.general.bit_perfect.idle",           @"On. Nothing is playing.", @"Settings, General pane: caption under the bit-perfect output switch while it is on and playback is stopped or paused.")
#define STR_SETTINGS_BIT_PERFECT_ACTIVE        NSLS(@"settings.general.bit_perfect.active",         @"Active: %@",               @"Settings, General pane: caption under the bit-perfect output switch while the current track is delivered bit-perfect. %@ is the format, e.g. '96.0 kHz · 24-bit · exclusive'.")
...
#define STR_SETTINGS_BIT_PERFECT_FORMAT_SHARED_DEFAULT NSLS(@"settings.general.bit_perfect.format_shared_default", @"%1$@ · %2$@-bit · shared, because it is the system output device", @"The output format bit-perfect output settled on, on a device the app deliberately does not take exclusive use of because it is the system's output device, so other apps' sounds can still mix in: %1$@ is the sample rate, already formatted with its unit (e.g. '96.0 kHz'), %2$@ the word length in bits.")
#define STR_SETTINGS_BIT_PERFECT_RATE_UNSUPPORTED NSLS(@"settings.general.bit_perfect.rate_unsupported", @"The device does not offer this file's sample rate; playing at %@", @"Bit-perfect output status: the device cannot run at the file's sample rate, so the audio is resampled. %@ is the rate the device is running at, already formatted with its unit (e.g. '88.2 kHz').")
#define STR_SETTINGS_BIT_PERFECT_SWITCH_FAILED NSLS(@"settings.general.bit_perfect.switch_failed",  @"Could not confirm the device's audio settings", @"Bit-perfect output status: the output route, format or volume/balance/mute settings could not be confirmed.")
#define STR_SETTINGS_BIT_PERFECT_DEPTH         NSLS(@"settings.general.bit_perfect.depth",          @"This file's word length is longer than the playback path carries", @"Bit-perfect output status: the file carries more bits per sample than the decoder or the device can pass on, so the audio is truncated. The decoder is 32-bit float, which carries 24 integer bits.")
...
#define STR_SETTINGS_BIT_PERFECT_MUTED         NSLS(@"settings.general.bit_perfect.muted",          @"The output device is muted. Unmute it in Sound settings.", @"Bit-perfect output status: the device's separate mute control is on, even if its volume still reads 100 percent.")
#define STR_SETTINGS_BIT_PERFECT_VOLUME        NSLS(@"settings.general.bit_perfect.volume",         @"The device's volume is %@%%. Set it to 100%% in Sound settings.", @"Bit-perfect output status: macOS is scaling the audio in software because the device's volume is below full. %@ is the volume as a whole-number percentage; %% is a literal percent sign.")
...
#define STR_SETTINGS_BIT_PERFECT_LOSSY         NSLS(@"settings.general.bit_perfect.lossy",          @"This file is lossy; the decoded audio is delivered unchanged", @"Bit-perfect output status: the file is a lossy format (MP3, AAC), so bit-perfect delivery applies to the decoded audio, not to a lossless original.")
