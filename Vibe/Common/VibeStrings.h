...
#define STR_SETTINGS_DECLICK                       NSLS(@"settings.general.declick", @"Declick", @"Settings, Audio pane: switch beneath Exclusive output. On, bit-perfect output ramps the volume over the first and last few milliseconds of every start, stop, pause, seek and track change so the edge cannot click; off, it cuts, leaving every sample untouched. A short noun phrase for removing clicks.")
#define STR_SETTINGS_DECLICK_CAPTION               NSLS(@"settings.general.declick.caption", @"Uses volume ramps to reduce clicking", @"Settings: declick explanation while bit-perfect output is on. No trailing period.")
#define STR_SETTINGS_DECLICK_ALWAYS_ON             NSLS(@"settings.general.declick.always_on", @"Always on while bit-perfect output is off", @"Settings: why the declick switch is disabled while bit-perfect output is off — ordinary playback always ramps, so the choice applies only to bit-perfect output. No trailing period.")
