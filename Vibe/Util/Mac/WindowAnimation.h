...
// content-size changes — runs at this one duration: the main window through
// its animationResizeTime: override, the settings window through its pane-owned
// NSAnimationContext. AppKit's default scales with the distance, at roughly
// 0.2s per 150pt, which makes larger jumps drag. These are chrome snapping to a
// new shape rather than content transitions, so a short fixed time reads better
// and stays consistent whatever the distance.
