...
- **The mac theme reaches the widget as values, not as a theme.** The publisher resolves `AppSettings.currentTheme` into the snapshot's `theme` dictionary (per-appearance palettes, the transport glyphs a theme changed) and the placeholder images, republishing only when one of those moves. The widget's own tile is dark; only a theme that paints the surface (solid background) gets its light side, its resolved label colors and a light waveform strip — otherwise just the dark side's set colors apply, so a color picked for a light window never lands on the dark tile. Nothing here is on iOS, which has no themes.
