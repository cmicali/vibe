...
**A track with no decoded art publishes the shell's placeholder**, which the driver hands in as `placeholderArt:` because only the shell knows it: the current theme's `resolvedDefaultArtworkImage` on macOS, `record-bg` (the pager's) on iOS. It goes through the same rasterization. The dirty check compares identity, so a theme change republishes and an unchanged theme does not; a two-sided theme's placeholder is drawn for the system appearance at publish time and is not redrawn on an appearance flip alone.
