...
**The thumbnail is a macOS representation, and iOS keeps none.** It exists for the playlist table's art cells, which iOS has no equivalent of — the pager shows the full-size decode and nothing standing in for it (`Vibe/iOS/CLAUDE.md`). So on iOS `embeddedThumbnail` is always nil, `prewarmEmbeddedThumbnail` is a no-op and the archive carries no JPEG: a track's art is read and decoded exactly once, at full size, for the page that shows it, instead of paying a 128px decode per scanned track, a re-encode per cache write and an eager decode per cache hit for an image nothing there draws. A scan on iOS therefore does no image work at all, and its cache entries are about 700 bytes rather than 5-20KB.

That is why **`hasEmbeddedArt` is archived as a flag of its own** rather than inferred from the thumbnail's presence, which is what the archive used to mean by it: with no thumbnail written, every iOS cache hit would read as artless — no art on the page, ever, and on macOS a folder cover handed to a file that has its own. Entries written before the flag carry no key and fall back to the old inference, so both forms read on both platforms. It is the `known` column of the state table in `AudioTrackArtwork.h`, and no discard clears it: the bytes go, the fact that the file has art does not.
