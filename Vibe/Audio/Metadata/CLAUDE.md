...
playlist cell draw ───→ thumbnailAlbumArt          ──→ folder-art resolver, serial queue
...
### TRAP: `albumArt` names two opposite contracts, one hop apart

```
AudioTrack.albumArt          → metadata.albumArtIfLoaded   NON-BLOCKING, decoded-only
AudioTrackMetadata.albumArt  → artwork.albumArt            BLOCKING: file read + decode
```

The main thread calls the first on every `updateUI`. Calling the second there reads an audio file, which a cloud placeholder can stall for minutes. `ArtworkDisplayController` is the only caller of the blocking one, always on a background queue, gated by `albumArtNeedsLoad` and the in-flight `albumArtLoadDispatched` flag.
...
- The **album-art setting** goes through `MainPlayerController.refreshFolderArt` → `albumArtSettingDidChange`, which re-reads the setting (it is cached — see below) and drops the decoded images, worth some 20MB and useless while it is off. The *answers* stay, because the setting governs whether the fallback is consulted at all and never what a folder contains. Wiping them meant that turning the feature off and on again demoted every walked folder to the lone-file stat probes, which know three of the candidate names — so a `cover.png` library lost its art to a round trip through the checkbox.
