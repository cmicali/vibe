...
`ArtworkDisplayController` is the **only** caller of `loadArtBlocking`, always on a user-initiated queue, gated by the two flags above. `AudioTrack` deliberately exposes no blocking accessor at all.
