...
**`setCloudParsesHeld:` closes that lane outright from play submission until the open settles** — the foreground open outranks every background parse that would download something. Local parses and both lanes' stage-1 cache checks are untouched, so rows keep filling from cache while the current track lands. Both shells acquire the hold in the player's synchronous pre-submit edge (`willSubmitPlayForTrack:`), before the open can contend with anything the hold suspends, and release it exactly once at the matching settlement: `didStartPlaying:`'s successor-prefetch claim acknowledgement (`prefetchTrack:whenClaimed:` — so the lane cannot resume before the prefetch's claim exists and race it to the same file), the error path, or Close. **The flag lives on the cache, not the loader**, because `loadMetadata:` mints a fresh loader — a sweep starting mid-open must not lose the hold.
...
**Known residual: everything above rests on `SF_DATALESS`.** Which lane a miss takes is decided by `NSURLUtil.isDatalessFile:` (in `cacheCheckOneTrack:`, and again in `loadSingleTrack:`'s own skip), so a provider-backed placeholder whose flag is never set reads as a local file: its parse goes to the wide lane, which the hold does not suspend, and a folder of them fills every row while the picked track waits — the exact starvation the hold exists to prevent. There is no second, provider-independent gate. This is a deliberate choice rather than an oversight: one gate is cheaper than two, and **no unflagged placeholder has been observed** — iCloud Drive and Dropbox both flag every evicted file, measured across whole trees. The debug channel can stage the shape (`set_fake_cloud … unflagged`, and the cloud scenario suite's S9 demonstrates the consequence on demand) and `set_dataless_diag` measures any new provider. If one ever proves unflagged, the fix is directory-scoped classification routing unknown misses onto this lane — never a per-file resource-value probe, whose cost and staleness `NSURLUtil.m` records.
...
**Because the lane is serial, its order is the whole of what it decides — and it is explicit.** Pending parses are plain app-owned records, never pre-submitted operations; the lane dispatches exactly one at a time and picks the next under the lock by `(deferred, neighborhood rank, playlist index)` — `CloudParseOrderRules.h`, tested — so the tail follows stable playlist order instead of stage-1 completion order, and an entry leaves the list only at dispatch, which makes everything pending re-rankable by construction.

**That last rule is what makes the pick's cost matter, so the picker selects rather than sorts.** Everything pending stays pending, so the list is as long as the playlist's misses — a real Dropbox tree opened as one playlist left 104,000 entries — and `setNeighborhoodURLs:` runs the picker on the **main thread** at every track change. Sorting that list, with each entry's rank looked up by `indexOfObject:` inside the comparator, measured ~300ms per track change against 49ms on a small playlist. One pass keeping the best `kCloudParsePickWindow` entries, each rank looked up once, brings it back to the small-playlist cost. The window only has to outnumber the paths the player can be materializing at once — a playback open and its prefetch, so two. The neighborhood is next, then the one after, then the one behind; the ranking arrives through `AudioTrackMetadataCache.setNeighborhoodAroundIndex:inTracks:`, sent from each shell's one current-index funnel — `PlaybackController.updateMetadataNeighborhood` on iOS, `PlaylistController.currentIndexDidChangeHandler` on the mac — and the picker reads it live, so a move re-ranks for free. **The offsets live on the cache, not in the shells**, because a shell can silently fail to supply them. Two more picker rules: it waits for the sweep's stage-1 checks to drain, or the hold's release inside `didStartPlaying:` races the enqueue burst and picks from a half-populated list; and it **skips an entry whose path the player or its prefetch is already materializing** (`AudioFileOpenCoordinator.isMaterializingURL:`), re-checked at run time with the entry standing aside at its rank, no attempt spent — a lane parked on nothing-but-blocked entries re-asks on a bounded 1s clock, since nothing else kicks it when the player's transfer settles. A timed-out pick is ranked back in first through `prependNeighborhoodURL:`, so the file the user asked for keeps downloading behind its error UI — but **only one that was still moving when the deadline ran out** (`kVibeAudioErrorOpenMadeProgressKey`). A transfer that never showed a byte is left an ordinary sweep candidate: re-fetching it would spend the provider's next slot, unasked and behind a terminal error, on a retry that would fail the same way.

**TRAP: the hold must close the gate before it cancels, never after.** The cancelled parse re-queues itself, and with the gate open its replacement would dispatch and start the same download again immediately — a cancel loop rather than a hold. `cancel` drops the pending records directly; nothing is pre-submitted, so nothing suspended can pin the discarded playlist, and the one dispatched parse settles its own flag on the way out.
