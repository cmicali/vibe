...
The files here are the iOS counterpart of `Audio/Mac/Devices/`: what answers when the system moves the audio out from under the engine, which on iOS is the carrier of Vibe's render pipeline (`Audio/CLAUDE.md`). The shared `(Carrier)` declaration in `AudioPlayerInternal.h` is implemented here for iOS; `(Recovery)` receives the session controller’s recovery verdicts.
