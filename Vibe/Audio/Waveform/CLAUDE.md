...
`AudioWaveform` is a C++ structure storing, per chunk, the peak min/max and the energy — a sum of squared samples plus a frame count, so merging partial buffers into a chunk and chunks into a drawn column stays exact in any order. `AVFAudioWaveformLoader`, reading through `AudioFileHandle` (`Audio/Loading/`), generates the data asynchronously and hands immutable snapshots to the main thread for progressive rendering. `AudioWaveformCache` owns the loader lifecycle and persists through PINCache.
