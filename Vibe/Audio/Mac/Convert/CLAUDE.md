...
**The encoder is CoreAudio's** (`AudioFileHandle`'s writing side, an ExtAudioFile created as `kAudioFileFLACType`): no new dependency, nothing for `THIRD-PARTY-NOTICES.md`. ffmpeg was never an option — the app is sandboxed and ships in the App Store.

`FLACConvertRules.h` holds the preliminary eligibility and naming rules as static inlines, so callers share the same decisions. The tests also compile the converter itself, replacing only its file-operation boundaries and keeping TagLib out. Eligibility is WAV and AIFF only: the sniffed `metadata.fileType` decides once it exists; the extension keeps the menu available before the background scan reaches a freshly dropped row. Acceptance always performs a RIFF/FORM header sniff on the converter queue.

**The FLAC's declared source bit depth is the file format's flag** (`kAppleLosslessFormatFlag_16BitSourceData` or the 24-bit one), and the buffers must be that wide: `encodeSource:` picks both from the source's own depth (`Int16` when not float and ≤16 bits, `Int32` and 24 otherwise), or a 16-bit recording is written as a 24-bit file that is bit-exact but misdescribes itself and is larger. The source is probed for depth, then reopened with that integer processing format; the waveform loader’s removed preflight does not eliminate this separate precision choice. Float is the one lossy case — FLAC stores integers, and every FLAC encoder quantizes float to 24 bits. The files are byte-identical to what `AVAudioFile`'s writer produced for the same buffers (measured, 16 and 24 bits).
...
Settlement detaches the completed request’s cancel waiters **before** calling its completion, then delivers those waiters afterward. A completion may accept another conversion and add its own cancel waiter; the old request must never consume it.
...
Placement gets one final positive audio validation before success reaches the controller. It runs off-main on the disposal queue (`playableFileAtURL:`): one `AudioFileHandle` open that decodes at least one frame, the proof that this process opened a regular nonempty file and CoreAudio accepted its container. If a provider or external move makes the placed FLAC unavailable in that gap, the source and playlist stay untouched. The placed path is preserved and its full absolute path is logged: a save-panel placement may have replaced a user file, and even a silent placement can be externally replaced before cleanup, so provenance is not sufficient authority to delete whatever is there now.

`progressHandler` reports the encode on the main thread at about one-percent steps, with the converting track so the owner can tell whether it is on screen. It drives the waveform's brush-through sweep (`WaveformUI/Renderers/CLAUDE.md`).
...
`registerUndoForConversion:undoManager:swap:completion:` owns both directions of the file round trip on the window's `NSUndoManager`, using the existing `VibeFLACConversionRecord`. The shell supplies only the row swap and UI settlement. The shared inverse restores the replacement when its Trash URL is known, positively verifies it, swaps, then trashes the retired file. Redo restores the FLAC rather than re-encoding, and re-trashes the source only when `sourceWasTrashed` says the accepted conversion did. Disposal failure still commits a verified row swap; a missing restore URL fails closed. A retired file already away from its expected path makes the inverse a no-op, protecting unrelated files now occupying that path.

**TRAP: each invocation registers its opposite synchronously, before both the async move and the in-flight refusal.** Registration during an async completion would go onto the wrong stack; skipping it on refusal would lose the popped action. `isUndoRedoInFlight` is shared across all records and clears before settlement. `FLACDisposalRulesTests` exercises the actual manager and converter with held restore/verify/trash callbacks, including failures and recovery.

All disposal and verification calls run on their own serial dispose queue, not the converter queue — an undo never waits out an encode, and a move or read a cloud folder makes unbounded stays off main; completions land on main. Both moves run inside coordinated writes, because a FLAC only the related-item rung could place lives at a path whose sandbox extension rides the presenter's file coordination — a bare move there is denied. The restore's move refuses to overwrite. The verifier is deliberately positive: a destructive handoff requires proof that the file opened and that the accepted container carries audio, not the absence of a refusal.
