...
// + W boost) and the send/returns (E reverb wash, R/T ping-pong delays).
// Each is driven by a bare key that both taps and holds — TransportKeyMonitor
// owns that distinction; this class just holds plain on/off state per effect.
// Owns the FX segment of the engine graph —
...
// the bass (Q key). A deck control — it persists across tracks and applies to
// whatever is (or starts) playing. Toggling sweeps the cutoff over ~80ms
// rather than switching instantly, so it never clicks.
//
// Setting this NO also clears lowKillBoostActive: the boost is a modifier of
// this filter, so it never outlives it.
@property (nonatomic) BOOL lowKillEnabled;

// Boost of the low kill (W key): while YES the same high-pass runs at DOUBLE
// the usual cutoff, whether or not lowKillEnabled is on; clearing it sweeps
// back to whatever lowKillEnabled implies. Same declick sweep as the toggle.
// Subordinate to lowKillEnabled — see its note above.
@property (nonatomic) BOOL lowKillBoostActive;

// Reverb send (E key): while YES, the master signal also feeds a long,
// fully-wet reverb return (low-cut so the tail can't muddy the bass).
// Setting NO cuts only the send — the tail rings out naturally.
@property (nonatomic) BOOL reverbSendEnabled;

// Delay echo send (R key): while YES, the master signal also feeds an
// 1/8-note ping-pong echo with aggressive feedback, high-passed so the
// repeats don't stack up bass. Setting NO cuts only the send — the trail
// decays through the feedback naturally.
@property (nonatomic) BOOL delaySendEnabled;

// Short delay echo send (T key): the same ping-pong echo as
