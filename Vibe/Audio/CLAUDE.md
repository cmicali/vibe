...
The controller calls `prefetchTrack:` on every track start to pre-open the likely next track; the next `play:` of that path consumes the parked `AVAudioFile`. The `whenClaimed:` variant acknowledges, once on main, that the prefetch's open claim is registered with the coordinator (or that none is needed) — registration, never admission, so a parked admission cannot delay it. The shells release the metadata cloud-lane hold in that acknowledgement, which is what orders "the successor's claim exists" before "the background lane resumes"; the acknowledgement can outlive a rapid next, so its handler stale-guards on the shell's hold generation — and the settlement that issues it is itself dropped when superseded, per the paragraph below, which is what makes the generation it captures the right one. The engine stops when playback goes idle (Stopped or Paused), releasing the output device, deferred ~6s and cancelled by generation in `startEngineAndPlayNode:`.
...
**macOS runs no FFT, and that is a runtime fact rather than a compile-time one.** `AudioLevelTap` builds into both targets; what keeps it off the mac is that nothing there ever sets `levelsEnabled` — the mac rows leave `levelSource` nil and run the canned keyframes, which Core Animation composites on the render server for no per-frame CPU at all. So the mac is not "gated", it is never armed, and there is nothing for a visibility gate to switch. The shared `dump_levels` verb is what makes that checkable instead of merely asserted: on macOS it must read `levelsEnabled: false` while a track plays with the playlist open, forever.
...
**Progress is matched by PATH, not by the open's identifier, and that is checked rather than assumed.** Elsewhere in this file the rule is that a URL is not an identity — a replay of the same row defeats every content-based guard, which is why settlement carries a submission identifier. It does not extend to the progress feed, for three reasons that have to fail together before a stale sample reaches the deadline: `monitorReplacing:` cancels the outgoing monitor, the movement block is `currentURL`-guarded inside the monitor, and `noteOpenProgressForURL:` no-ops unless a pending request has that exact path. Past all three, a sample can only *extend* a deadline for an open of the same path — which a same-path rebind means is usually the same underlying transfer — so the worst case is abandoning a wedged open 20s late.
