...
**TRAP: offline rendering can outrun the node's asynchronous file read.** Before manual playback starts, `startEngineAndPlayNode:` prepares one maximum render block with `prepareWithFrameCount:`; before every frame-driven slice, `VibeManualRenderPump` lands the node's worker, which performs the start and every later read (`Debug/CLAUDE.md`). The bounded pre-roll keeps a cold replay from consuming the capture window as silence; hardware playback keeps its normal asynchronous start.
