...
The controller calls `prefetchTrack:` on every track start to pre-open the likely next track; the next `play:` of that path consumes the parked `AVAudioFile`. The `whenClaimed:` variant acknowledges, once on main, that both the purpose-keyed open claim and its path-wide materialization request are atomically registered (or that none is needed) — registration, never worker admission, so a parked admission cannot delay it. Gapless has no materialization request and acknowledges immediately. A central rejection settles the acknowledgement as cancelled rather than claiming work that never registered; the player treats that terminal verdict as safe to release. A different-path prefetch requested while playback is still opening is retained but suppressed: successful playback resumes that exact target and waits for its claim registration, while failure, abandonment, a newer explicit play (same-path rebind included), or a later prefetch terminally retires it and acknowledges once. Request identity drops every late claim callback. The shells release the metadata background-materialization hold in that acknowledgement, which is what orders "the successor's claim exists" before "the background scan resumes"; a terminal acknowledgement can outlive a rapid next, so its handler stale-guards on the shell's hold generation. The engine stops when playback goes idle (Stopped or Paused), releasing the output device, deferred ~6s and cancelled by generation in `startEngineAndPlayNode:`.

At play submission, an unrelated old prefetch is cancelled before the foreground open starts, so its provider transfer cannot compete with the file the user chose. A same-path prefetch stays alive: its purpose-keyed handle claim and the playback claim remain independent, but both join one standardized-path materialization operation. When that operation reports Ready, their existing schedulers open separate `AVAudioFile` handles; whichever succeeds first consumes the play request, and the winner retires the loser's park state so the now-current track cannot become its own prefetched successor. A later consecutive row with the same path is a fresh `prefetchTrack:` and remains valid.

**A settlement for a superseded submission is never delivered, and the shells could not decide this for themselves.** `didStartPlaying:` and the play-path errors are both dropped on main when a newer play has been submitted since — `submittedPlayIsCurrent:`, checked inside the delivery block, because what matters is whether a newer play existed by the time the callback *ran*. The coordinator's own rule ("a superseded open's result is never consumed") cannot cover this: once a result has been consumed and dispatched to main, nothing can retract it, and a play submitted in that window leaves the shell holding a callback for the previous play.
...
Why it has to be identity and not the track: a **replay of the same row is the same `AudioTrack` and the same URL**, so every stale guard the shells have reads the old play's callback as current. Measured, that let a stale `didStartPlaying:` re-run its whole tail — including the successor prefetch whose acknowledgement releases the metadata materialization hold, stamped with the *newer* play's generation — and the background scan resumed 15ms into an open the user was still waiting on.

`playTrack:` also fires the one delegate send not marshalled to main: `willSubmitPlayForTrack:`, synchronous on `play:`'s calling thread and ahead of the queue dispatch — the pre-submit edge where the shells assert the background-materialization hold before the open can contend with anything the hold suspends. It fires for every play, fast paths and rebinds included; the handler must not block or call back into the player.
...
`AudioFileMaterializationCoordinator` owns the one operation that can make a standardized path local, with Playback, Prefetch and the two metadata roles affecting admission rather than identity. `AudioFileOpenCoordinator` stays above it: one current claim per `(purpose, path)` preserves independent `AVAudioFile` handles and delivery, while playback maps to the central Playback role, prefetch maps to Prefetch, and the private gapless handle bypasses materialization because the parked file already proved the bytes local. Only central Ready submits an `AVAudioFile` open; Yielded, AdmissionExhausted and Failed become terminal outer errors without touching AVFoundation.

Playback handle opens use a fixed-slot user-initiated scheduler; prefetch and the private gapless handle use a separate utility scheduler, and waveform work has two of its own. Pending blocks stay in explicitly bounded app memory and are not dispatched until a slot is free, and every admission failure is delivered on the caller's nominated failure queue — never inline, so a rejection decided while the caller sits on a serial queue cannot re-enter it. Within each purpose, the coordinator keeps one standardized-path claim registered until an uncancellable `AVAudioFile` call really returns. A timeout or superseding request detaches both its outer delivery and its central materialization waiter; a handle open which already began keeps the outer claim until it returns, so repeated retries cannot multiply stranded workers.
...
Admission has its own failure at both bounded stages. The central coordinator applies the immutable configuration's path-transfer limits, pending bounds and grace; after Ready, the handle-open scheduler still protects AVFoundation from a never-returning open. Either admission exhaustion maps to `VibeAudioFileOpenErrorAdmissionExhausted`, saying the later file never began its next stage rather than reporting that file's ordinary playback timeout. A true never-returning OS call cannot be killed in-process and keeps its fixed slot until process restart, but the schedulers prevent that unavoidable loss from becoming unbounded worker or queue growth.
