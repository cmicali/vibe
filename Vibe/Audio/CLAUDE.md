...
The lane slot a transfer held is **carried** through its purposes' handle opens and released when the path's last run returns, so a wedged `AVAudioFile` call stays inside the same admission that bounded its transfer — which is why the foreground lane is three wide, not two: one wedge must not halve foreground capacity. Waveform work keeps its own two fixed-slot schedulers. Within each purpose, the coordinator keeps one run registered until an uncancellable `AVAudioFile` call really returns. A timeout or superseding request detaches both its outer delivery and its materialization waiter; a handle open which already began keeps its run until it returns, so repeated retries cannot multiply stranded workers.
...
Admission fails once, at stage 1: the coordinator applies the immutable configuration's path-transfer limits, pending bounds and grace — `AudioWorkAdmissionRules.h`'s `VibeAudioWorkAdmission` is that decision and `VibeAudioWorkAdmissionExpired` the pending-grace clock, both tested, and `AudioWorkScheduler` reads the same pair — and exhaustion maps to `VibeAudioFileOpenErrorAdmissionExhausted` — the later file never began, rather than that file's ordinary playback timeout. A true never-returning OS call cannot be killed in-process and keeps its carried slot until process restart, but the lane bound prevents that unavoidable loss from becoming unbounded worker or queue growth.
