...
**State is `{Stopped, Playing, Paused, Loading}`, and Loading keeps its intent live.** Loading covers the in-flight open and reports zero position and duration; play/pause during it toggles whether the opened file starts or parks (on iOS, how an interruption mid-load avoids starting the engine against an inactive session), and a seek replaces its start position. Exactly one of `isPlaying`/`isPaused`/`isStopped` is true; `isLoading` is orthogonal. An action that must order itself after already-submitted pause/resume commands calls `playingIntentAfterPendingCommands` once, an action-only queue barrier, so the ordinary UI getters stay lock-only.
...
**A track change crossfades on two independent chains, and a live node is never rerouted**, because reconnecting one reconfigures the graph and clicks. Each track gets its own `AVAudioUnitVarispeed`, minted in `playOnQueue:`; the outgoing node fades out on its own and is detached, varispeed and all, once silent.
