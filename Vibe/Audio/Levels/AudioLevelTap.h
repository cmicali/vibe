...
//  for its lifetime and replaces only the tap when the output is rebuilt or
//  the normalization mode changes.
//
...
// installs the pointer before its gate opens and retires it only once no
// render is inside (AudioPlayer+Graph.h), so the render never reads memory
// the tap has freed.
typedef struct VibeLevelMeter VibeLevelMeter;

@interface AudioLevelTap : NSObject

// Allocates the analyzer for `format`'s rate and begins a publisher session.
// The delivered rate remains authoritative: the render's first block at
// another rate rebinds the analyzer without allocating. Returns nil for an
// unusable format or a failed allocation. The normalization mode is fixed for
// the tap's lifetime; replace the tap to switch modes and reset its analysis
// history.
...
// completes a pending signal capture. Player queue, once the render has been
// seen outside the meter.
- (void)remove;

// Beta probe of the existing tap, bounded to first signal or three seconds.
// All calls and completion belong to the player queue. Poll returns YES while
// pending; removal and replacement also complete partial captures. No call
// creates demand or opens a file. The last snapshot survives removal.
...
// channels of the final output at `sampleRate`, the block stamped
