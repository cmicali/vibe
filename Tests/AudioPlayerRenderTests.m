...
static NSDictionary *ComparePCM(NSData *reference, NSData *capture, NSUInteger channels,
    ...
    if (!channels) return @{@"pass": @NO, @"reason": @"No channels"};
    ...
    return @{@"pass": @(mismatches == 0), @"mismatchedSamples": @(mismatches), @"firstBadFrame": @(first),
...
- (void)testOracleRejectsCorruption {
    ...
    for (NSString *mutation in @[@"drop",@"duplicate",@"swap",@"polarity",@"gain",@"clip",@"truncate",@"silence",@"nan",@"lsb",@"shortMatch"]) {
...
- (void)checkLossy:(NSString *)name {
    ...
    [self play:url paused:NO position:0];
    [self assertReference:PCM(decoded) capture:[self renderSeconds:decoded.frameLength/_rate+0.1] skip:(NSUInteger)(_rate*0.05) tolerance:0];
}
- (void)testAACContainer { [self checkLossy:@"lossy.m4a"]; }
- (void)testAACElementary { [self checkLossy:@"lossy.aac"]; }
- (void)testMP4 { [self checkLossy:@"alias.mp4"]; }
- (void)testMP3CBR { [self checkLossy:@"cbr.mp3"]; }
- (void)testMP3VBR { [self checkLossy:@"vbr.mp3"]; }
- (void)testMP2 { [self checkLossy:@"lossy.mp2"]; }
- (void)testQuickTimeAudio { [self checkLossy:@"lossy.qta"]; }
